...
const SEARCH_STOPWORDS = new Set([
  ...
  'earlier', 'last', 'remember', 'remembered', 'remembering', 'recall', 'recalled', 'recalling',
  'remind', 'reminded', 'said', 'talk', 'talked', 'talking',
  'time', 'today', 'yesterday', 'system', 'thing', 'things'
]);

const STRONG_RECENCY_PATTERN = /\b(?:earlier|today|yesterday|last\s+(?:time|conversation|session)|recent(?:ly)?|we\s+(?:talked|discussed)|you\s+said\s+earlier|do\s+you\s+(?:remember|recall)|remind\s+me)\b/i;
...
function countTermOccurrences(text, term) {
  ...
  if (normalizedTerm.length > 3) {
    ...
    if (normalizedTerm.endsWith('y')) variants.add(`${normalizedTerm.slice(0, -1)}ies`);
  }
  let count = 0;
...
function scoreCandidate(candidate, querySpec, options = {}) {
  ...
  const expansionSources = querySpec.expansionSources && typeof querySpec.expansionSources === 'object'
    ? querySpec.expansionSources
    : {};
  const independentExpansionMatches = expandedMatches.filter(term => {
    const sourceIds = Array.isArray(expansionSources[term]) ? expansionSources[term] : [];
    return !sourceIds.length || sourceIds.some(id => String(id) !== String(candidate.id || ''));
  });
  const occurrenceTotal = [...primaryMatches, ...contextMatches, ...independentExpansionMatches]
    .reduce((sum, term) => sum + Math.min(3, countTermOccurrences(searchText, term)), 0);
  const expansionCoverage = expandedTerms.length ? independentExpansionMatches.length / expandedTerms.length : 0;
  ...
  return {
    ...
    workspaceScore,
    totalScore,
    matchedTerms: [...new Set([...primaryMatches, ...contextMatches, ...independentExpansionMatches])]
  };
}

function compareRankedEvidence(a, b) {
  return ((b.candidate.sourceKind === 'conversation' ? 1 : 0) - (a.candidate.sourceKind === 'conversation' ? 1 : 0))
...
function collectExpansionTerms(initialRanked, querySpec, limit = MAX_EXPANDED_TERMS) {
  ...
  initialRanked.slice(0, 4).forEach((ranked, index) => {
    ...
    for (const term of terms) {
      ...
      if (!sources.has(term)) sources.set(term, new Set());
      sources.get(term).add(String(ranked.candidate.id || ''));
    }
  });
  ...
  return {
    terms,
    sources: Object.fromEntries(terms.map(term => [term, [...(sources.get(term) || [])]]))
  };
}
...
function searchConversationEvidence(candidates, options = {}) {
  ...
  const expansion = collectExpansionTerms(initialRanked, querySpec, options.maxExpandedTerms);
  const expandedTerms = expansion.terms;
  const expandedQuery = {
    ...querySpec,
    expandedTerms,
    expansionSources: expansion.sources,
