...
const chromeFavorites = require('./chrome-favorites');
...
function buildOpenApplicationScript(appName) {
  ...
  return `$ErrorActionPreference = 'Stop'\n` +
    ...
    `  [pscustomobject]@{ success=$true; method='activated'; effect='activated_existing'; appName=$name; processName=$chosen.ProcessName; windowTitle=$chosen.MainWindowTitle } | ConvertTo-Json -Compress\n` +
    `  exit 0\n` +
    `}\n` +
    ...
    `if ($startApps.Count -eq 0) { [pscustomobject]@{ success=$false; notFound=$true; reasonCode='application_not_found'; error="No installed Start-menu application matched '$name'."; matches=@() } | ConvertTo-Json -Compress; exit 0 }\n` +
    `if ($startApps.Count -gt 1) { [pscustomobject]@{ success=$false; ambiguous=$true; reasonCode='application_ambiguous'; error="More than one installed application matched '$name'. Use a more specific display name."; matches=@($startApps | Select-Object -First 8 -ExpandProperty Name) } | ConvertTo-Json -Compress; exit 0 }\n` +
    `$selected = $startApps[0]\n` +
    `Start-Process explorer.exe -ArgumentList ("shell:AppsFolder\\" + $selected.AppID)\n` +
    `[pscustomobject]@{ success=$true; method='launched'; effect='opened_new'; appName=$selected.Name; appId=$selected.AppID } | ConvertTo-Json -Compress\n`;
}
...
function normalizeAccessibleUiAction(input = {}) {
  const targetText = String(input.targetText || '').normalize('NFKC').trim();
  if (!targetText) throw new Error('click_ui_element requires targetText.');
  if (targetText.length > 300) throw new Error('Accessible UI target text is limited to 300 characters.');
  const appName = String(input.appName || '').normalize('NFKC').trim();
  if (appName.length > 120) throw new Error('Application names are limited to 120 characters.');
  const controlType = String(input.controlType || '').normalize('NFKC').trim();
  if (controlType.length > 80) throw new Error('Control types are limited to 80 characters.');
  const matchMode = String(input.matchMode || 'exact').trim().toLowerCase();
  if (!['exact', 'contains'].includes(matchMode)) throw new Error('matchMode must be exact or contains.');
  return {
    targetText,
    appName,
    controlType,
    matchMode,
    occurrence: boundedInteger(input.occurrence, 'occurrence', 1, 25, 1),
    occurrenceSpecified: input.occurrence !== undefined && input.occurrence !== null && input.occurrence !== '',
    settleMs: boundedInteger(input.settleMs, 'settleMs', 0, 10000, 900)
  };
}

function buildAccessibleUiActionScript(input = {}) {
  const normalized = normalizeAccessibleUiAction(input);
  const payload = powershellLiteralBase64(JSON.stringify(normalized));
  return `$ErrorActionPreference = 'Stop'\n` +
    `Add-Type -AssemblyName UIAutomationClient\n` +
    `Add-Type -AssemblyName UIAutomationTypes\n` +
    `Add-Type -TypeDefinition @'\n` +
    `using System;\nusing System.Runtime.InteropServices;\n` +
    `public static class OrionAccessibleInput {\n` +
    ...
    `$handle = [OrionAccessibleInput]::GetForegroundWindow()\n` +
    `if ($request.appName) {\n` +
    `  $matches = @(Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 -and ($_.ProcessName.IndexOf([string]$request.appName, [StringComparison]::OrdinalIgnoreCase) -ge 0 -or $_.MainWindowTitle.IndexOf([string]$request.appName, [StringComparison]::OrdinalIgnoreCase) -ge 0) })\n` +
    `  if ($matches.Count -eq 0) { [pscustomobject]@{ success=$false; notFound=$true; reasonCode='application_window_not_found'; error="No visible application window matched '$($request.appName)'. Use open_application first." } | ConvertTo-Json -Compress; exit 0 }\n` +
    `  $handle = @($matches | Sort-Object @{Expression={ if ($_.ProcessName -ieq $request.appName -or $_.MainWindowTitle -ieq $request.appName) { 0 } else { 1 } }}, StartTime | Select-Object -First 1)[0].MainWindowHandle\n` +
    `  [OrionAccessibleInput]::SetForegroundWindow($handle) | Out-Null\n` +
    `}\n` +
    `$window = [System.Windows.Automation.AutomationElement]::FromHandle($handle)\n` +
    `if (-not $window) { [pscustomobject]@{ success=$false; reasonCode='no_accessibility_tree'; error='The foreground window does not expose an accessibility tree.' } | ConvertTo-Json -Compress; exit 0 }\n` +
    `$candidates = [System.Collections.Generic.List[object]]::new()\n` +
    `$all = $window.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)\n` +
    `foreach ($element in $all) {\n` +
    `  try {\n` +
    `    $name = [string]$element.Current.Name; $automationId = [string]$element.Current.AutomationId\n` +
    `    $typeName = [string]$element.Current.ControlType.ProgrammaticName; if ($typeName.StartsWith('ControlType.')) { $typeName = $typeName.Substring(12) }\n` +
    `    if ($request.controlType -and -not [String]::Equals($typeName, [string]$request.controlType, [StringComparison]::OrdinalIgnoreCase)) { continue }\n` +
    `    $nameExact = [String]::Equals($name, [string]$request.targetText, [StringComparison]::OrdinalIgnoreCase)\n` +
    `    $idExact = [String]::Equals($automationId, [string]$request.targetText, [StringComparison]::OrdinalIgnoreCase)\n` +
    `    $contains = ($name.IndexOf([string]$request.targetText, [StringComparison]::OrdinalIgnoreCase) -ge 0 -or $automationId.IndexOf([string]$request.targetText, [StringComparison]::OrdinalIgnoreCase) -ge 0)\n` +
    `    if ($request.matchMode -eq 'exact' -and -not ($nameExact -or $idExact)) { continue }\n` +
    `    if ($request.matchMode -eq 'contains' -and -not $contains) { continue }\n` +
    `    if ($element.Current.IsOffscreen -or -not $element.Current.IsEnabled) { continue }\n` +
    `    $score = if ($nameExact) { 0 } elseif ($idExact) { 1 } else { 2 }\n` +
    `    $rect = $element.Current.BoundingRectangle\n` +
    `    $candidates.Add([pscustomobject]@{ Element=$element; Score=$score; Name=$name; AutomationId=$automationId; ControlType=$typeName; Left=[int]$rect.Left; Top=[int]$rect.Top; Width=[int]$rect.Width; Height=[int]$rect.Height })\n` +
    `  } catch {}\n` +
    `}\n` +
    `$ordered = @($candidates | Sort-Object Score, Top, Left)\n` +
    `$best = if ($ordered.Count -gt 0) { @($ordered | Where-Object { $_.Score -eq $ordered[0].Score }) } else { @() }\n` +
    `if ($best.Count -gt 1 -and -not [bool]$request.occurrenceSpecified) {\n` +
    `  $summary = @($best | Select-Object -First 8 Name,AutomationId,ControlType,Left,Top,Width,Height)\n` +
    `  [pscustomobject]@{ success=$false; ambiguous=$true; reasonCode='control_ambiguous'; error="More than one visible accessible control matched '$($request.targetText)'. Supply occurrence or a narrower control type/app name."; matches=$summary } | ConvertTo-Json -Compress -Depth 4; exit 0\n` +
    `}\n` +
    `if ($best.Count -lt [int]$request.occurrence) {\n` +
    `  $summary = @($best | Select-Object -First 8 Name,AutomationId,ControlType,Left,Top,Width,Height)\n` +
    `  [pscustomobject]@{ success=$false; notFound=$true; reasonCode='control_not_found'; error="No unique visible accessible control matched '$($request.targetText)' at occurrence $($request.occurrence)."; matches=$summary } | ConvertTo-Json -Compress -Depth 4; exit 0\n` +
    `}\n` +
    `$selected = $best[[int]$request.occurrence - 1]; $chosen = $selected.Element; $method = ''\n` +
    `try { $pattern = $chosen.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern); ([System.Windows.Automation.InvokePattern]$pattern).Invoke(); $method='invoke' } catch {}\n` +
    `if (-not $method) { try { $pattern = $chosen.GetCurrentPattern([System.Windows.Automation.SelectionItemPattern]::Pattern); ([System.Windows.Automation.SelectionItemPattern]$pattern).Select(); $method='select' } catch {} }\n` +
    `if (-not $method) { try { $pattern = $chosen.GetCurrentPattern([System.Windows.Automation.ExpandCollapsePattern]::Pattern); ([System.Windows.Automation.ExpandCollapsePattern]$pattern).Expand(); $method='expand' } catch {} }\n` +
    `if (-not $method) {\n` +
    `  if ($selected.Width -le 0 -or $selected.Height -le 0) { [pscustomobject]@{ success=$false; reasonCode='control_not_clickable'; error='The matched control has no clickable bounds.' } | ConvertTo-Json -Compress; exit 0 }\n` +
    `  $x = [int]($selected.Left + ($selected.Width / 2)); $y = [int]($selected.Top + ($selected.Height / 2))\n` +
    `  [OrionAccessibleInput]::SetCursorPos($x, $y) | Out-Null; [OrionAccessibleInput]::mouse_event(0x0002,0,0,0,[UIntPtr]::Zero); [OrionAccessibleInput]::mouse_event(0x0004,0,0,0,[UIntPtr]::Zero); $method='bounded-click'\n` +
    `}\n` +
    `[pscustomobject]@{ success=$true; method=$method; effect='control_activated'; name=$selected.Name; automationId=$selected.AutomationId; controlType=$selected.ControlType; bounds=[pscustomobject]@{ left=$selected.Left; top=$selected.Top; width=$selected.Width; height=$selected.Height } } | ConvertTo-Json -Compress -Depth 4\n`;
}

async function performAccessibleUiAction(input = {}, options = {}) {
  const normalized = normalizeAccessibleUiAction(input);
  const runUiAction = options.runUiAction
    || (action => runPowerShellJson(buildAccessibleUiActionScript(action), options.execFile || execFile));
  return runUiAction(normalized);
}

// Resolves which physical display an action/capture should target. An omitted display means the
// primary display. An explicit display is evidence-bound: if that monitor disappeared after the
// model inspected it, silently acting on primary would target an uninspected surface.
function resolveTargetDisplay(electronScreen, displayId) {
  ...
  const error = new Error(`Display ${displayId} is no longer connected. Capture and inspect a fresh screen before acting.`);
  error.code = 'STALE_DISPLAY_ID';
  error.availableDisplayIds = all.map(display => String(display.id));
  throw error;
}
...
function registerHandlers(ipcMain, options = {}) {
      ...
      if (typeof captureDesktopScreenshot === 'function') {
        try {
          shot = await captureDesktopScreenshot(payload.workspacePath || '', payload.destination || '', 'application', {
            ...
            windowHint: applicationResult.windowTitle || applicationResult.appName || payload.appName || ''
          });
        } catch (error) {
          captureError = String(error && error.message || error || 'Screen capture failed.');
        }
      }
      return {
        ...applicationResult,
        ...
        displayId: payload.displayId || '',
        captureMode: shot ? shot.captureMode || 'display' : '',
        captureSuccess: !!shot,
        captureError: captureError || undefined,
        summary: shot
          ? `${applicationResult.effect === 'activated_existing' || applicationResult.method === 'activated' ? 'Activated' : 'Opened'} "${applicationResult.appName || payload.appName}" and captured its visible window (${shot.size.width}x${shot.size.height}).`
          : `${applicationResult.effect === 'activated_existing' || applicationResult.method === 'activated' ? 'Activated' : 'Opened'} "${applicationResult.appName || payload.appName}", but could not capture verification evidence: ${captureError || 'screen capture unavailable'}.`
      };
    } catch (error) {
  ...
  ipcMain.handle('click-accessible-ui', async (event, payload = {}) => {
    if (process.platform !== 'win32' && !options.runUiAction) {
      return { success: false, error: 'Accessible desktop control is currently supported only on Windows.' };
    }
    const conversationId = String(payload.conversationId || '');
    if (!conversationId) return { success: false, error: 'Accessible desktop control requires an owning Operator conversation.' };
    const mainWindow = shared.mainWindow;
    const shouldHide = !!(mainWindow && !mainWindow.isDestroyed() && !operatorControlSession.isControlSessionActive());
    if (shouldHide) {
      mainWindow.hide();
    ...
    try {
      const actionResult = await performAccessibleUiAction(payload, options);
      if (!actionResult || actionResult.success === false) return actionResult;
      const settleMs = boundedInteger(payload.settleMs, 'settleMs', 0, 10000, 900);
      if (settleMs > 0) await new Promise(resolve => setTimeout(resolve, settleMs));
      // Same split as open_application and open_chrome_favorite. The window hint is the app the
      // control lives in; when no app was named there is genuinely no window to fall back to, so
      // the hint stays empty rather than guessing at one.
      let shot = null;
      let captureError = '';
      if (typeof captureDesktopScreenshot === 'function') {
        try {
          shot = await captureDesktopScreenshot(payload.workspacePath || '', payload.destination || '', 'accessible-ui', {
            hideOrion: false,
            conversationId,
            displayId: payload.displayId || '',
            windowHint: actionResult.appName || payload.appName || ''
          });
        } catch (error) {
          captureError = String(error && error.message || error || 'Screen capture failed.');
        }
      ...
      return {
        ...
        captureError: captureError || undefined,
        summary: shot
          ? `Activated the accessible control "${actionResult.name || payload.targetText}" and captured the resulting screen (${shot.size.width}x${shot.size.height}).`
          : `Activated the accessible control "${actionResult.name || payload.targetText}", but could not capture verification evidence: ${captureError || 'screen capture unavailable'}.`
      };
    } catch (error) {
      return { success: false, error: error.message || 'Accessible desktop control failed.' };
    } finally {
      if (shouldHide) {
        if (typeof mainWindow.showInactive === 'function') mainWindow.showInactive();
        else mainWindow.show();
  ...
  ipcMain.handle('open-chrome-favorite', async (event, payload = {}) => {
    if (process.platform !== 'win32' && !options.openFavorite) {
      return { success: false, error: 'Chrome favorite control is currently supported only on Windows.' };
    }
    const conversationId = String(payload.conversationId || '');
    if (!conversationId) return { success: false, error: 'Opening a Chrome favorite requires an owning Operator conversation.' };
    const mainWindow = shared.mainWindow;
    const shouldHide = !!(mainWindow && !mainWindow.isDestroyed() && !operatorControlSession.isControlSessionActive());
    if (shouldHide) {
      mainWindow.hide();
    ...
    try {
      const resolved = typeof options.resolveChromeFavorite === 'function'
        ? await options.resolveChromeFavorite(payload)
        : chromeFavorites.resolveChromeFavorite(payload, options.chromeFavoritesOptions || {});
      if (!resolved || resolved.success === false) return resolved;
      const favorite = resolved.favorite;
      const opened = typeof options.openFavorite === 'function'
        ? await options.openFavorite(favorite)
        : await runPowerShellJson(chromeFavorites.buildOpenChromeUrlScript(favorite.url), options.execFile || execFile);
      if (!opened || opened.success === false) return opened;
      const settleMs = boundedInteger(payload.settleMs, 'settleMs', 0, 10000, 1400);
      if (settleMs > 0) await new Promise(resolve => setTimeout(resolve, settleMs));
      // Opening the favorite and photographing the result are separate facts, the same way
      // open_application already treats them. Before this, a capture failure propagated to the
      // outer catch and the whole call reported "Captured screen image was empty" — reading as
      // though the favorite had not opened when it had. The window hint matters just as much:
      // captureDesktopScreenshot's native named-window fallback only engages when it is told what
      // window to look for, and a favorite opens in Chrome. Without it, an unavailable desktop
      // framebuffer had no recovery path at all.
      let shot = null;
      let captureError = '';
      if (typeof captureDesktopScreenshot === 'function') {
        try {
          shot = await captureDesktopScreenshot(payload.workspacePath || '', payload.destination || '', 'chrome-favorite', {
            hideOrion: false,
            conversationId,
            displayId: payload.displayId || '',
            windowHint: 'Chrome'
      ...
      return {
        ...
        captureError: captureError || undefined,
        summary: shot
          ? `Opened Chrome favorite "${favorite.name}"${favorite.folder ? ` from ${favorite.folder}` : ''} and captured the resulting screen (${shot.size.width}x${shot.size.height}).`
          : `Opened Chrome favorite "${favorite.name}"${favorite.folder ? ` from ${favorite.folder}` : ''}, but could not capture verification evidence: ${captureError || 'screen capture unavailable'}.`
      };
    } catch (error) {
      return { success: false, error: error.message || 'Chrome favorite control failed.' };
    } finally {
      if (shouldHide) {
        if (typeof mainWindow.showInactive === 'function') mainWindow.showInactive();
        else mainWindow.show();
...
module.exports = {
  ...
  normalizeAccessibleUiAction,
  buildAccessibleUiActionScript,
  performAccessibleUiAction,
