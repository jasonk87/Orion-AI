...
function normalizeComputerAction(input = {}, display = {}) {
  ...
  if (!['move', 'click', 'scroll', 'type', 'key'].includes(action)) {
    throw new Error('computer_action requires action: move, click, scroll, type, or key.');
  }
  ...
  if (['move', 'click', 'scroll'].includes(action)) {
...
async function performComputerAction(input, options = {}) {
  ...
  const display = {
    bounds: primary.bounds || { x: 0, y: 0, width: primary.size.width, height: primary.size.height },
    sourceWidth: Math.round(primary.size.width * scale),
    sourceHeight: Math.round(primary.size.height * scale)
  };
  ...
  return { ...result, normalizedAction: action };
}
...
function registerHandlers(ipcMain, options = {}) {
  ...
  ipcMain.handle('computer-action', async (event, payload = {}) => {
    ...
    const shouldHide = !!(mainWindow && !mainWindow.isDestroyed());
    if (shouldHide) {
      mainWindow.hide();
      await new Promise(resolve => setTimeout(resolve, 180));
    }
    try {
      const actionResult = await performComputerAction(input, options);
