...
function buildAccessibleUiActionScript(input = {}) {
  ...
  return `$ErrorActionPreference = 'Stop'\n` +
    ...
    `  if ($matches.Count -eq 0) { [pscustomobject]@{ success=$false; error="No visible application window matched '$($request.appName)'. Use open_application first." } | ConvertTo-Json -Compress; exit 0 }\n` +
    ...
    `if (-not $window) { [pscustomobject]@{ success=$false; error='The foreground window does not expose an accessibility tree.' } | ConvertTo-Json -Compress; exit 0 }\n` +
    ...
    `  [pscustomobject]@{ success=$false; ambiguous=$true; error="More than one visible accessible control matched '$($request.targetText)'. Supply occurrence or a narrower control type/app name."; matches=$summary } | ConvertTo-Json -Compress -Depth 4; exit 0\n` +
    `}\n` +
    `if ($best.Count -lt [int]$request.occurrence) {\n` +
    `  $summary = @($best | Select-Object -First 8 Name,AutomationId,ControlType,Left,Top,Width,Height)\n` +
    `  [pscustomobject]@{ success=$false; error="No unique visible accessible control matched '$($request.targetText)' at occurrence $($request.occurrence)."; matches=$summary } | ConvertTo-Json -Compress -Depth 4; exit 0\n` +
    `}\n` +
    ...
    `if (-not $method) {\n` +
    `  if ($selected.Width -le 0 -or $selected.Height -le 0) { [pscustomobject]@{ success=$false; error='The matched control has no clickable bounds.' } | ConvertTo-Json -Compress; exit 0 }\n` +
    ...
    `}\n` +
    `[pscustomobject]@{ success=$true; method=$method; name=$selected.Name; automationId=$selected.AutomationId; controlType=$selected.ControlType; bounds=[pscustomobject]@{ left=$selected.Left; top=$selected.Top; width=$selected.Width; height=$selected.Height } } | ConvertTo-Json -Compress -Depth 4\n`;
}
