  ...
  ipcMain.handle('click-accessible-ui', async (event, payload = {}) => {
    ...
    try {
      ...
      return {
        ...
        size: shot ? shot.png.length : 0,
        displayId: payload.displayId || '',
        summary: `Activated the accessible control "${actionResult.name || payload.targetText}"${shot ? ` and captured the resulting screen (${shot.size.width}x${shot.size.height}).` : '.'}`
      };
    } catch (error) {
  ...
  ipcMain.handle('open-chrome-favorite', async (event, payload = {}) => {
    ...
    try {
      ...
      return {
        ...
        size: shot ? shot.png.length : 0,
        displayId: payload.displayId || '',
        summary: `Opened Chrome favorite "${favorite.name}"${favorite.folder ? ` from ${favorite.folder}` : ''}${shot ? ` and captured the resulting screen (${shot.size.width}x${shot.size.height}).` : '.'}`
      };
    } catch (error) {
