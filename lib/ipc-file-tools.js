...
const { readMultipleRanges, inspectCodeContext } = require('./context-retrieval');
...
function registerHandlers(ipcMain) {
  ...
  ipcMain.handle('read-multiple-ranges', async (event, { workspacePath, files, options }) => {
    try {
      return readMultipleRanges(workspacePath, files, options || {});
    } catch (e) {
  ...
  ipcMain.handle('orion:inspect-code-context', async (event, payload = {}) => {
    try {
      return await inspectCodeContext(payload.workspacePath, payload);
    } catch (e) {
  ...
  ipcMain.handle('orion:assign-context-packets', async (event, payload = {}) => {
    try {
      const service = getWorkspaceIndexService(payload.workspacePath);
      return service.assignContextPackets(payload.packetIds, payload);
    } catch (e) {
  ...
  ipcMain.handle('orion:hydrate-context-packets', async (event, payload = {}) => {
    try {
      const service = getWorkspaceIndexService(payload.workspacePath);
      return service.hydrateContextPackets(payload.packetIds, payload);
    } catch (e) {
  ...
  ipcMain.handle('orion:get-file-symbols', async (event, { workspacePath, relativePath }) => {
    try {
      return {
        success: true,
        symbols: getWorkspaceIndexService(workspacePath).getFileSymbols(relativePath)
      };
    } catch (e) {
      return { success: false, error: e.message };
    }
