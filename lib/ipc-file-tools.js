...
const { SCAN_SKIP_DIRECTORIES } = require('./scan-ignore');

const BINARY_EXTENSIONS = new Set([
  '.png', '.jpg', '.jpeg', '.gif', '.ico', '.bmp', '.webp', '.pdf',
  '.zip', '.gz', '.tar', '.7z', '.rar', '.glb', '.fbx', '.exe', '.dll', '.so', '.dylib',
  '.woff', '.woff2', '.ttf', '.eot', '.mp3', '.mp4', '.wav', '.mov', '.avi',
  '.db', '.sqlite', '.node'
]);
const IMAGE_EXTENSIONS = new Set(['.png', '.jpg', '.jpeg', '.gif', '.ico', '.bmp', '.webp']);

function getBinaryReadError(relativePath) {
  const ext = path.extname(String(relativePath || '')).toLowerCase();
  if (!BINARY_EXTENSIONS.has(ext)) return '';
  if (IMAGE_EXTENSIONS.has(ext)) {
    return `read_file only reads text; ${relativePath} is an image. Use inspect_screenshot_with_model with this path and a visual goal, or inspect_binary_asset for metadata.`;
  }
  return `read_file only reads text; ${relativePath} is a binary asset. Use inspect_binary_asset or the appropriate archive/asset tool instead.`;
}
...
function readWorkspaceFileBase64(workspacePath, relativePath, conversationId = '') {
  const scopedConversationId = String(conversationId || '').trim();
  const artifactRef = parseArtifactRef(relativePath);
  if (scopedConversationId && artifactRef && artifactRef.conversationId !== sanitizeArtifactSegment(scopedConversationId)) {
    throw new Error('Conversation artifact belongs to a different conversation');
  }
  ...
  if (!fullPath && path.isAbsolute(relativePath)) {
    ...
    if (relativePath.startsWith(artifactRoot)) {
      if (scopedConversationId) {
        const conversationRoot = path.resolve(getConversationArtifactDir(scopedConversationId));
        const candidate = path.resolve(relativePath);
        if (candidate !== conversationRoot && !candidate.startsWith(conversationRoot + path.sep)) {
          throw new Error('Conversation artifact belongs to a different conversation');
        }
      }
  ...
  // A bare filename or subpath (e.g. the `destination` a model passed to take_screenshot /
  // capture_screen, rather than the orion-artifact:// URI those tools actually returned in their
  // result) should still resolve on the first try when that conversation already has an artifact
  // by that name. writeScreenshotBuffer (lib/ipc-shell.js) always writes screenshots into the
  // conversation's own artifact directory whenever a conversationId is supplied, so a model that
  // reuses its own destination string instead of the returned path was guaranteed a "File does not
  // exist" error and a mandatory retry on every single screenshot-then-inspect-or-attach call.
  // Checking the conversation's artifact directory first (before falling back to the workspace)
  // mirrors where these tools actually write, and only kicks in when nothing else already resolved
  // the path, so a genuine workspace-relative read is unaffected.
  if (!fullPath && scopedConversationId && !artifactRef && !path.isAbsolute(relativePath)) {
    try {
      const candidate = resolveConversationArtifactPath(scopedConversationId, relativePath);
      if (fs.existsSync(candidate) && fs.statSync(candidate).isFile()) fullPath = candidate;
    } catch (_) { /* not a valid artifact-relative path; fall through to workspace resolution */ }
  }
...
function applyPatch(original, operation) {
  ...
  if (operation.type === 'replace') {
    ...
    const firstIndex = updated.indexOf(operation.target);
    if (firstIndex === -1) throw new Error('Target content block not found');
    if (count === 1 && updated.indexOf(operation.target, firstIndex + operation.target.length) !== -1) {
      throw new Error('Target content block is not unique; it appears multiple times. Use a more specific target, or use replace_range only after re-reading the current line numbers.');
    }
    let replacements = 0;
...
function registerHandlers(ipcMain) {
  ...
  ipcMain.handle('list-files', async (event, dirPath) => {
    try {
      ...
      const skipped = new Set(['node_modules', '.git', 'dist', '.gemini', 'build', '.orion', '.claude']);
      // Bounds, because this walk is unbounded otherwise and its inputs are not under our
      // control: a workspace can contain a deeply nested dependency tree, a generated artifact
      // directory with hundreds of thousands of files, or a pathological path. Recursing
      // without limits risks a stack overflow that takes down the main process, and building an
      // unbounded array risks exhausting memory — both from merely LOOKING at a directory.
      // Truncation is reported so the caller shows a real limit rather than a quiet half-list.
      const MAX_DEPTH = 24;
      const MAX_ENTRIES = 50000;
      let truncated = false;
      let totalEntries = 0;
      const getFiles = (dir, rootDir, depth = 0) => {
        let results = [];
        if (depth > MAX_DEPTH) { truncated = true; return results; }
        // A workspace is routinely mutated while it is being walked — an agent run installing
        // packages, a build writing output, a cleanup deleting a tree. readdir followed by lstat
        // is inherently racy there, so a single entry that vanishes or is locked must skip that
        // entry, not abort the whole listing and fail the caller.
        let list;
        try {
          list = fs.readdirSync(dir);
        } catch (dirError) {
          console.error(`Skipping unreadable directory ${dir}:`, dirError.message);
          return results;
        }
        for (const file of list) {
          // Checked per entry rather than only on entry, so an enormous single directory is
          // capped too — not just a deep tree.
          if (totalEntries >= MAX_ENTRIES) { truncated = true; break; }
          ...
          if (stat.isDirectory()) {
            ...
            totalEntries++;
            results = results.concat(getFiles(filePath, rootDir, depth + 1));
          } else {
      ...
      const files = getFiles(dirPath, dirPath);
      if (truncated) {
        // Carried as a marker entry rather than changing the return type: every caller already
        // treats a non-array as a failure, and a silent half-listing is worse than a visible
        // limit. The agent's list_files surfaces this so it does not conclude a file is absent.
        files.truncated = true;
        files.truncationReason = `Listing stopped at ${MAX_ENTRIES} entries or depth ${MAX_DEPTH}. This directory is larger or deeper than a full listing supports, so results are incomplete — narrow the path or use search instead of concluding a file does not exist.`;
      }
      return files;
    } catch (e) {
  ...
  ipcMain.handle('grep-search', async (event, { workspacePath, pattern, options }) => {
    try {
      ...
      const contextLines = Number.isFinite(Number(opts.contextLines)) ? Math.max(0, Math.min(10, Number(opts.contextLines))) : 0;
      ...
      // Models routinely write `foo|bar|baz` alternation without setting regex:true. Literal mode
      // used to match the pipe as an ordinary character, so those searches silently returned zero
      // matches — indistinguishable from genuine absence, and the documented source of an agent
      // run confidently reporting "this handler/CSS doesn't exist" about code that does. Literal
      // mode now treats `|` as OR over literal alternatives.
      const normalizedPattern = caseSensitive ? String(pattern) : String(pattern).toLowerCase();
      const needles = normalizedPattern.split('|').map(part => part.trim()).filter(Boolean);
      if (needles.length === 0) needles.push(normalizedPattern);

      const results = [];
      let filesScanned = 0;
      let truncated = false;
      let budgetExhausted = false;
      const truncateLine = (line) => line.length > 300 ? line.slice(0, 300) + '…' : line;

      // This walk runs in the Electron MAIN process. A fully synchronous crawl blocked the event
      // loop for the entire scan — across a Dispatch workspace rooted at the whole Projects
      // folder that meant tens of seconds with no window message pump and an OS-level
      // "not responding" dialog. So: yield to the event loop every few dozen files, and stop at a
      // wall-clock budget rather than grinding through an unbounded tree.
      const SCAN_TIME_BUDGET_MS = 10000;
      const YIELD_EVERY_FILES = 50;
      const deadline = Date.now() + SCAN_TIME_BUDGET_MS;
      let filesSinceYield = 0;

      const walk = async (dir, rootDir) => {
        if (truncated || budgetExhausted) return;
        let entries;
        try {
        ...
        for (const entry of entries) {
          if (truncated || budgetExhausted) return;
          if (SCAN_SKIP_DIRECTORIES.has(entry)) continue;
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (matcher) {
              ...
              isMatch = needles.some(n => haystack.includes(n));
            }
            if (isMatch) {
              const match = {
                path: relPath,
              ...
              if (contextLines > 0) {
                ...
                match.context = lines.slice(start, end + 1).map((contextLine, offset) => {
                  const contextIndex = start + offset;
                  return {
                    line: contextIndex + 1,
                    text: truncateLine(contextLine),
      ...
      // A zero-match literal search whose pattern contains regex metacharacters is far more
      // likely a mode mistake than genuine absence — say so instead of letting the model
      // conclude "doesn't exist" from a search that never could have matched.
      let literalModeNote;
      if (!opts.regex && results.length === 0 && /[\\^$*+?()[\]{}]/.test(String(pattern))) {
        literalModeNote = `Zero matches, and the pattern contains regex metacharacters that were matched as LITERAL text ('|' alternation is supported, but \\b, .*, (), [] etc. are not). If you meant a regex, retry with regex: true. Do not conclude the code is absent from this result alone.`;
      }
      ...
      return {
        ...
        message: budgetExhausted
          ? `Search stopped after ${SCAN_TIME_BUDGET_MS / 1000}s (${filesScanned} files scanned, ${results.length} matches so far). The workspace is too large for this pattern — narrow it with filePattern, a more specific pattern, or search inside one project directory instead of the workspace root.`
          : (truncated ? `Truncated at ${maxResults} matches — narrow the pattern or add filePattern if you need more.` : literalModeNote)
      };
    } catch (e) {
  ...
  ipcMain.handle('read-file', async (event, { workspacePath, relativePath, options }) => {
    try {
      const fullPath = resolveWorkspacePath(workspacePath, relativePath);
      if (!fs.existsSync(fullPath)) throw new Error('File does not exist');
      const binaryReadError = getBinaryReadError(relativePath);
      if (binaryReadError) return { error: binaryReadError, binary: true, path: relativePath };
      const content = fs.readFileSync(fullPath, 'utf8');
      return applyReadOptions(content, options || {});
    } catch (e) {
  ...
  ipcMain.handle('read-multiple-ranges', async (event, { workspacePath, files, options }) => {
    try {
      return await requestWorkspaceIndex('readMultipleRanges', { workspacePath, files, options: options || {} });
    } catch (e) {
      console.error('Error reading multiple ranges:', e);
      return { success: false, error: e.message };
    }
  });

  ipcMain.handle('orion:inspect-code-context', async (event, payload = {}) => {
    try {
      return await requestWorkspaceIndex('inspectCodeContext', payload);
    } catch (e) {
      console.error('Error inspecting code context:', e);
      return { success: false, error: e.message };
    }
  });

  ipcMain.handle('orion:assign-context-packets', async (event, payload = {}) => {
    try {
      return await requestWorkspaceIndex('assignContextPackets', payload);
    } catch (e) {
      console.error('Error assigning context packets:', e);
      return { success: false, error: e.message, assignedPacketIds: [] };
    }
  });

  ipcMain.handle('orion:hydrate-context-packets', async (event, payload = {}) => {
    try {
      return await requestWorkspaceIndex('hydrateContextPackets', payload);
    } catch (e) {
      console.error('Error hydrating context packets:', e);
      return { success: false, error: e.message, sections: [], content: '' };
    }
  });
  ...
  ipcMain.handle('edit-config', async (event, { workspacePath, relativePath, updates }) => {
    try {
      if (!workspacePath || !relativePath || !updates || typeof updates !== 'object') {
        return { success: false, error: 'Missing required parameters or updates is not an object.' };
      }
      // 'path' is not required in ipc-file-tools.js natively at this exact line but fs/path are required at the top usually
      const p = require('path');
      const fullPath = p.join(workspacePath, relativePath);
      if (!fs.existsSync(fullPath)) return { success: false, error: 'Config file not found: ' + relativePath };
      ...
      return { success: false, error: 'Failed to edit config: ' + e.message };
    }
  });
  ...
  ipcMain.handle('read-workspace-file-base64', async (event, { workspacePath, relativePath, conversationId }) => {
    try {
      return readWorkspaceFileBase64(workspacePath, relativePath, conversationId);
    } catch (e) {
      return { success: false, error: e.message };
    }
  });
  ...
  ipcMain.handle('orion:get-file-symbols', async (event, { workspacePath, relativePath }) => {
    try {
      return await requestWorkspaceIndex('getFileSymbols', { workspacePath, relativePath });
    } catch (e) {
      return { success: false, error: e.message };
    }
  });
  ...
  ipcMain.handle('orion:semantic-search', async (event, { query, workspacePath, config, topK }) => {
    try {
      return await requestWorkspaceIndex('semanticSearch', { query, workspacePath, config, topK });
    } catch (e) {
      return { success: false, error: e.message };
    }
  });

  ipcMain.handle('orion:fetch-docs', async (event, { libraryName, version, url }) => {
    try {
      const os = require('os');
      const docsPath = path.join(os.homedir(), 'orion-docs-cache', `${libraryName}@${version}`);
      ...
      const filename = url.split('/').pop() || 'docs.md';
      const safeFilename = filename.replace(/[^a-zA-Z0-9.-]/g, '_') || 'docs.md';
      const finalFilename = safeFilename.endsWith('.md') || safeFilename.endsWith('.json') || safeFilename.endsWith('.txt') ? safeFilename : `${safeFilename}.md`;
      const filePath = path.join(docsPath, finalFilename);
      
      fs.writeFileSync(filePath, content, 'utf8');
      
      return { success: true, message: `Docs fetched and saved to ${filePath}` };
    } catch (e) {
      return { success: false, error: e.message };
    }
  });

  ipcMain.handle('orion:search-docs', async (event, { libraryName, version, query, config }) => {
    try {
      const os = require('os');
      const docsPath = path.join(os.homedir(), 'orion-docs-cache', `${libraryName}@${version}`);
      if (!fs.existsSync(docsPath)) return { success: false, error: 'Docs not cached yet. Fetch them first.' };
      const { semanticSearch } = require('./semantic-search');
      const results = await semanticSearch(query, docsPath, config, 10);
...
function applyReadOptions(content, options = {}) {
  ...
  const maxChars = parseInt(options.maxChars, 10);
  if (Number.isInteger(maxChars) && maxChars > 0 && selected.length > maxChars) {
    return selected.slice(0, maxChars)
      + `\n\n[Orion] Read truncated at ${maxChars} characters to protect the model context. Use inspect_code_context or narrower startLine/endLine ranges for the remaining exact source.`;
  }
  return selected;
}
