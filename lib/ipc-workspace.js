...
function listFilesRecursive(dirPath) {
  const getFiles = (dir, rootDir) => {
    ...
    list.forEach((file) => {
      ...
      if (['node_modules', '.git', 'dist', '.gemini', 'build', '.ruff_cache', '.pytest_cache', '__pycache__', '.venv', 'venv', 'env', '.tox', '.next', '.codex-remote-attachments', '.idea', '.vscode', 'coverage', '.orion'].includes(file)) return;
