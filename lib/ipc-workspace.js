...
// On Windows, a detached child that itself runs through cmd.exe (whether via our own wrapping or
// Node's shell:true) silently loses its piped stdout — verified directly: the process completes
// with exit code 0, but no output is ever delivered, even redirected to a file from inside the
// detached cmd.exe. A plain, single-level spawn of a real .exe does not have this problem. So for
// npm specifically, bypass the npm.cmd shim entirely and run its own CLI script directly through
// node.exe (bundled alongside every Node install) — a real .exe, avoiding both the ENOENT (npm.cmd
// not resolving) and the detached-stdout-loss (nested cmd.exe) problems at once.
function resolveDirectNpmInvocation(args) {
  if (process.platform !== 'win32') return null;
  try {
    const npmCliPath = path.join(path.dirname(process.execPath), 'node_modules', 'npm', 'bin', 'npm-cli.js');
    if (!fs.existsSync(npmCliPath)) return null;
...
function spawnInternalCommand(workspacePath, executable, args = []) {
  ...
  const directNpm = executable === 'npm' ? resolveDirectNpmInvocation(args) : null;
  ...
  // This fallback is only used when the direct npm-cli.js resolution above isn't available.
  const spawnExecutable = directNpm
    ? directNpm.executable
    : (process.platform === 'win32' ? resolveWindowsShellExecutable('cmd.exe') : executable);
  const spawnArgs = directNpm
    ? directNpm.args
...
function listFilesRecursive(dirPath) {
  const getFiles = (dir, rootDir) => {
    ...
    list.forEach((file) => {
      ...
      if (SCAN_SKIP_DIRECTORIES.has(file)) return;
...
async function getGeminiEmbedding(text, apiKey) {
  const model = resolveGeminiEmbeddingModel(readAppConfig());
...
async function runBackgroundIndexing(workspacePath, apiKey) {
  ...
  try {
    ...
    const embeddingModel = resolveGeminiEmbeddingModel(readAppConfig());
    let indexData = { workspace: workspacePath, embeddingModel, files: {}, chunks: [] };
    if (fs.existsSync(indexPath)) {
      try {
        indexData = JSON.parse(fs.readFileSync(indexPath, 'utf8'));
        // Vectors from a different embedding model are numerically incompatible with new query
        // vectors — comparing them yields garbage similarity, not degraded results. Rebuild.
        if (indexData.embeddingModel !== embeddingModel) {
          indexData = { workspace: workspacePath, embeddingModel, files: {}, chunks: [] };
        }
      } catch (_) {
        indexData = { workspace: workspacePath, embeddingModel, files: {}, chunks: [] };
      }
    }
    indexData.embeddingModel = embeddingModel;
...
function registerHandlers(ipcMain, { startStaticWorkspaceServer } = {}) {
  ...
  ipcMain.handle('git-diff', async (event, { workspacePath, relativePath }) => {
    try {
      if (!workspacePath || !fs.existsSync(workspacePath)) {
        return { success: false, error: 'No active workspace directory found.' };
      }
      ...
      const diffResult = await runGitCommand(workspacePath, args);
      return diffResult;
    } catch (e) {
      return { success: false, error: e.message };
    }
  ...
  ipcMain.handle('git-rollback', async (event, { workspacePath, relativePath }) => {
    try {
      if (!workspacePath || !fs.existsSync(workspacePath)) {
        return { success: false, error: 'No active workspace directory found.' };
      }
      if (!relativePath) {
        return { success: false, error: 'relativePath is required for rollback.' };
      }
      const rollbackResult = await runGitCommand(workspacePath, ['checkout', 'HEAD', '--', relativePath]);
  ...
  ipcMain.handle('search-embeddings', async (event, { query, limit }) => {
    try {
      ...
      // An index built with a different (e.g. retired) embedding model cannot be compared
      // against this query vector — treat it as absent and let background indexing rebuild it.
      if (indexData.embeddingModel !== resolveGeminiEmbeddingModel(config)) {
        runBackgroundIndexing(currentWorkspacePath, apiKey);
        return { success: true, results: [], message: 'The semantic index was built with an outdated embedding model and is being rebuilt. Retry search_embeddings shortly, or use grep_search/read_file meanwhile.' };
      }
