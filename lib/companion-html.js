...
const COMPANION_CLIENT_BUILD = 'ui-polish-v14';
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Skills tab ──────────────────────────────── */
    #panel-skills { overflow-y: auto; -webkit-overflow-scrolling: touch; padding: 12px; }
    #panel-skills::-webkit-scrollbar { width: 3px; }
    #panel-skills::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.08); border-radius: 2px; }
    .skill-card { background: var(--surface); border: 1px solid var(--line); border-radius: 10px; padding: 12px 13px; margin-bottom: 10px; }
    .skill-card-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 8px; margin-bottom: 6px; }
    .skill-card-name { font-size: 0.88rem; font-weight: 700; color: var(--text); }
    .skill-card-group { font-size: 0.68rem; font-weight: 600; padding: 2px 7px; background: rgba(130,115,244,0.14); color: var(--accent); border-radius: 4px; white-space: nowrap; }
    .skill-card-desc { font-size: 0.76rem; color: var(--muted); line-height: 1.4; margin-bottom: 10px; }
    .skill-run-btn { width: 100%; padding: 8px; background: linear-gradient(135deg, var(--accent), var(--accent-strong)); color: #fff; border: 0; border-radius: 8px; font-size: 0.8rem; font-weight: 700; cursor: pointer; font-family: inherit; transition: opacity 0.15s; }
    .skill-run-btn:disabled { opacity: 0.45; cursor: not-allowed; }
    .skill-inputs-form { margin: 8px 0 10px; display: flex; flex-direction: column; gap: 7px; }
    .skill-input-label { font-size: 0.72rem; color: var(--muted); font-weight: 600; margin-bottom: 2px; }
    .skill-input-field { width: 100%; box-sizing: border-box; background: var(--surface2); border: 1px solid var(--line); border-radius: 7px; color: var(--text); font-family: inherit; font-size: 0.78rem; padding: 7px 9px; outline: none; }
    .skill-result { margin-top: 8px; background: var(--surface2); border: 1px solid var(--line); border-radius: 8px; padding: 9px 11px; font-size: 0.73rem; color: var(--muted); white-space: pre-wrap; word-break: break-all; max-height: 120px; overflow-y: auto; }
    .skill-result.success { border-color: rgba(70,213,155,0.3); color: var(--success); }
    .skill-result.error { border-color: rgba(255,123,123,0.3); color: var(--danger); }
...
<div class="app-root">
  ...
  <div id="screen-main" class="screen">
    ...
    <div class="tab-panels">
      ...
      <div class="tab-panel active" id="panel-chat">
        ...
        <div class="composer-area">
          ...
          <input type="file" id="phone-img-input" accept="image/*" capture="environment" style="display:none" aria-hidden="true" tabindex="-1">
          ...
          <form id="prompt-form">
            <div class="composer">
              <button class="attach-image-btn" id="attach-image-btn" type="button" aria-label="Attach image" title="Attach image">&#128247;</button>
              <textarea id="prompt" placeholder="Ask Orion..." autocomplete="off" rows="1"></textarea>
              <button class="send-button" id="send" type="submit" aria-label="Send"></button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="tab-panel" id="panel-status">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; padding:8px 2px; margin-bottom:2px; gap:8px;">
          <span id="update-check-status" style="font-size:0.75rem; color:var(--muted); flex:1;">Checking for updates…</span>
          <button id="update-check-now-btn" type="button" style="padding:5px 12px; border-radius:8px; border:1px solid var(--line); background:rgba(255,255,255,0.05); color:var(--text); font-size:0.75rem; font-family:inherit; cursor:pointer;">Check Now</button>
          <button id="restart-app-btn" type="button" style="padding:5px 12px; border-radius:8px; border:1px solid var(--line); background:rgba(255,255,255,0.05); color:var(--text); font-size:0.75rem; font-family:inherit; cursor:pointer;">Restart</button>
        </div>
        ...
        <div class="update-banner" id="notif-banner" style="margin-top:6px; background:rgba(99,102,241,0.08); border-color:rgba(99,102,241,0.25); color:#a5b4fc;">
      ...
      <div class="tab-panel" id="panel-skills">
        <div class="section-header" style="margin-bottom:10px;">
          <div class="section-title">Skill Registry</div>
          <span class="badge muted" id="skills-count-badge">0 Skills</span>
          <button id="refresh-skills" type="button" class="btn-sm">&#x21BB;</button>
        </div>
        <div id="skills-list"><div class="empty">Loading skills&hellip;</div></div>
      </div>
    ...
    <nav class="bottom-nav">
      ...
      <button class="nav-btn" data-panel="panel-skills">
        <span class="nav-icon">&#x26A1;</span>
...
<script>
  ...
  // independent of whatever conversation happens to be open. Defaults to Dispatch.
  let companionMode = localStorage.getItem('orionCompanionMode') === 'coder' ? 'coder' : 'orion';
  ...
  function updateHomeScreen(state) {
    ...
    connTextEl.textContent = 'Connected to ' + machineName + ' · ' + companionBuild;
    ...
    homeRecentsTitleEl.textContent = companionMode === 'coder' ? 'Coder Chats' : 'Dispatch Chats';
  ...
  function renderProjectScreen() {
    const project = availableProjects.find(p => p.path === projectDetailPath) || { path: projectDetailPath, name: projectDetailPath ? projectDetailPath.split(/[\\/]/).pop() || projectDetailPath : 'Project' };
  ...
  homeClearProjectEl.addEventListener('click', () => {
    ...
    projSelectorNameEl.textContent = selectedProject
      ...
      : (newChatSelectedProject ? newChatSelectedProject.split(/[\\/]/).pop() || newChatSelectedProject : 'Standalone');
    updateHomeScreen({ conversations: allConversations, projects: availableProjects, globalRunning: false });
  });
  ...
  async function startNewPhoneChat(projectPath, promptText) {
    ...
    const res = await companionFetch('/api/conversations/new', {
      method: 'POST',
      body: JSON.stringify({ prompt: '', projectPath: projectPath || '', mode })
    });
    const data = await res.json();
  ...
  function sanitizeMarkdownHref(href) {
    ...
    return /^(https?:|mailto:|orion-file:)/i.test(value) ? value : '';
  }
  ...
  async function switchTask(taskId) {
    ...
    try {
      ...
      await loadState({ minSerial: serial });
  ...
  async function loadSkills() {
    try {
      const res = await companionFetch('/api/skills');
      const data = await res.json();
      skillsCache = Array.isArray(data.skills) ? data.skills : [];
      renderSkillsList(skillsCache);
    } catch (e) {
      if (skillsListEl) skillsListEl.innerHTML = '<div class="empty">Failed to load skills: ' + escapeHtml(e.message) + '</div>';
    }
  }

  function renderSkillsList(skills) {
    if (skillsCountBadge) skillsCountBadge.textContent = skills.length + ' Skill' + (skills.length !== 1 ? 's' : '');
    if (!skillsListEl) return;
    if (!skills.length) {
      skillsListEl.innerHTML = '<div class="empty">No skills registered yet. Orion creates skills automatically when it spots a reusable pattern.</div>';
      return;
    }
    skillsListEl.innerHTML = skills.map(skill => {
      const inputKeys = Object.keys(skill.inputs || {});
      const requiredKeys = inputKeys.filter(k => skill.inputs[k] && skill.inputs[k].required);
      const hasInputs = inputKeys.length > 0;
      const inputsHtml = hasInputs
        ? '<div class="skill-inputs-form" id="skill-inputs-' + escapeHtml(skill.name) + '">' +
            inputKeys.map(k => {
              const def = skill.inputs[k] || {};
              return '<div>' +
                '<div class="skill-input-label">' + escapeHtml(k) + (def.required ? ' *' : '') + '</div>' +
                '<input class="skill-input-field" data-skill="' + escapeHtml(skill.name) + '" data-key="' + escapeHtml(k) + '" type="text" placeholder="' + escapeHtml(def.description || '') + '" />' +
              '</div>';
            }).join('') +
          '</div>'
        : '';
      return '<div class="skill-card" data-skill-name="' + escapeHtml(skill.name) + '">' +
        '<div class="skill-card-header">' +
          '<span class="skill-card-name">' + escapeHtml(skill.displayName || skill.name) + '</span>' +
          '<span class="skill-card-group">' + escapeHtml(skill.group || '') + '</span>' +
        '</div>' +
        '<div class="skill-card-desc">' + escapeHtml(skill.description || '') + '</div>' +
        inputsHtml +
        '<button class="skill-run-btn" data-run-skill="' + escapeHtml(skill.name) + '">&#x25B6; Run</button>' +
        '<div class="skill-result" id="skill-result-' + escapeHtml(skill.name) + '" style="display:none;"></div>' +
      '</div>';
    }).join('');
  }

  skillsListEl && skillsListEl.addEventListener('click', async event => {
    const btn = event.target.closest('[data-run-skill]');
    if (!btn) return;
    const skillName = btn.getAttribute('data-run-skill');
    const skill = skillsCache.find(s => s.name === skillName);
    if (!skill) return;
    // Gather inputs
    const inputs = {};
    const form = document.getElementById('skill-inputs-' + skillName);
    if (form) {
      form.querySelectorAll('.skill-input-field').forEach(field => {
        const key = field.getAttribute('data-key');
        if (key && field.value.trim()) inputs[key] = field.value.trim();
      });
    }
    const resultEl = document.getElementById('skill-result-' + skillName);
    btn.disabled = true;
    btn.textContent = 'Running…';
    if (resultEl) { resultEl.style.display = 'none'; resultEl.className = 'skill-result'; resultEl.textContent = ''; }
    try {
      const res = await companionFetch('/api/skills/run', { method: 'POST', body: JSON.stringify({ name: skillName, inputs }) });
      const data = await res.json();
      if (resultEl) {
        resultEl.style.display = 'block';
        if (data.success) {
          resultEl.className = 'skill-result success';
          resultEl.textContent = JSON.stringify(data.outputs, null, 2);
        } else {
          resultEl.className = 'skill-result error';
          resultEl.textContent = data.error || 'Skill failed';
        }
      }
    } catch (e) {
      if (resultEl) { resultEl.style.display = 'block'; resultEl.className = 'skill-result error'; resultEl.textContent = e.message; }
    } finally {
      btn.disabled = false;
      btn.innerHTML = '&#x25B6; Run';
    }
  ...
  document.getElementById('refresh-skills') && document.getElementById('refresh-skills').addEventListener('click', loadSkills);

  // Load skills when the Skills tab is first opened
  navBtns.forEach(btn => {
    if (btn.dataset.panel === 'panel-skills') {
      btn.addEventListener('click', () => { if (!skillsCache.length) loadSkills(); });
    }
  });
  ...
  function renderConversationMessages(state) {
    ...
    const wasLoading = /Loading conversation/i.test(messagesEl.textContent || '');
    ...
    const renderedMessages = messages.map(msg => {
          ...
          const logsHtml = msg.role === 'assistant' ? renderToolCallRows(msg.logs || []) : '';
  ...
  function renderToolCallRows(toolCalls) {
    ...
    const rows = toolCalls.slice(-8).map(call => {
  ...
  async function emergencyFillMessages() {
    try {
      if (!messagesEl || !/Loading conversation/i.test(messagesEl.textContent || '')) return;
  ...
  async function loadState(options = {}) {
    ...
    if (Date.now() - lastSseMessageAt < 2500) return;
    try {
      if (!deviceSession) {
  ...
  function applyState(state) {
      ...
      chatProjNameEl.textContent = state.title || 'Chat';
      renderConversationMessages(state);
      ...
      const viewMode = state.mode || 'orion';
    ...
    try {
      if (formMode === 'steer') {
        ...
        if (!currentConversationId) {
          const newTaskRes = await companionFetch('/api/conversations/new', { method: 'POST', body: JSON.stringify({ prompt: '', projectPath: '', mode: companionMode }) });
        ...
        const res = await companionFetch('/api/prompt', { method: 'POST', body: JSON.stringify(promptPayload) });
        const data = await res.json();
  ...
  // ── Image attach ──────────────────────────────────
  const phoneImgInput    = document.getElementById('phone-img-input');
  ...
  function clearPhoneImage() {
    ...
    phoneImgThumb.src = '';
    phoneImgPreview.style.display = 'none';
    if (attachImageBtn) attachImageBtn.classList.remove('has-image');
    if (phoneImgInput) phoneImgInput.value = '';
  }

  if (attachImageBtn && phoneImgInput) {
    attachImageBtn.addEventListener('click', () => phoneImgInput.click());
    phoneImgInput.addEventListener('change', () => {
      const file = phoneImgInput.files && phoneImgInput.files[0];
      if (!file) return;
      ...
      reader.onload = e => {
        ...
        phoneImgThumb.src = dataUrl;
        phoneImgPreview.style.display = 'flex';
        attachImageBtn.classList.add('has-image');
      };
  ...
  async function checkForUpdate({ manual = false } = {}) {
    ...
    try {
      ...
      if (data.hasUpdate && updateBanner) {
        const commits = data.commitsBehind > 0 ? ' (' + data.commitsBehind + ' commit' + (data.commitsBehind === 1 ? '' : 's') + ' behind)' : '';
        if (updateBannerTxt) updateBannerTxt.textContent = 'Update available' + commits;
  ...
  async function setupPushNotifications() {
    ...
    if (Notification.permission === 'denied') { if (notifBanner) notifBanner.classList.remove('visible'); return; }
    ...
    if (existing) {
      ...
      await fetch('/api/push-subscribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + deviceSession, 'X-Orion-Device-Id': deviceId },
        body: JSON.stringify({ subscription: existing })
      }).catch(() => {});
      return;
    }
    // Show banner prompting user to enable
    if (notifBanner && Notification.permission !== 'granted') notifBanner.classList.add('visible');
    if (notifBanner && Notification.permission === 'granted') {
  ...
  // Check for git updates once after auth settles, then every 5 minutes
