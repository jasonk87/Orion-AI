...
const COMPANION_CLIENT_BUILD = 'dispatch-project-entry-v20';
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .message-answer th, .message-answer td { border: 1px solid var(--line); padding: 6px 8px; text-align: left; }
    .message-answer th { background: var(--surface2); font-weight: 600; }
    ...
    #screen-new-chat.dispatch-mode .dispatch-chat-intro { display: flex; }
    #screen-new-chat.dispatch-mode .coder-workspace-picker { display: none; }
    ...
    .message .message-answer h1 { font-size: 1.08rem; }
    .message .message-answer h2 {
      font-size: 1rem;
      padding-bottom: 6px;
      border-bottom: 1px solid var(--line);
    }
    .message .message-answer h3 { font-size: 0.94rem; color: #c8c0ff; }
    ...
    .dispatch-landing-intro { padding: 8px 2px 24px; }
    .dispatch-landing-title { color: var(--text); font-size: 1.25rem; font-weight: 800; }
    .dispatch-landing-copy { margin-top: 5px; color: var(--muted); font-size: 0.82rem; line-height: 1.45; }
    .dispatch-landing-context {
      display: inline-flex; align-items: center; gap: 6px; margin-top: 10px;
      color: #c4b5fd; font-size: 0.74rem; font-weight: 700;
    }
    .dispatch-landing-section { padding: 16px 0 2px; border-top: 1px solid var(--line); }
    ...
    .dispatch-work-row {
      display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 10px;
      width: 100%; min-height: 58px; padding: 10px 2px; border: 0;
    ...
    /* Message images */
    .message-image { max-width: 100%; max-height: 260px; border-radius: 10px; display: block; margin: 6px 0; border: 1px solid var(--line); }
    ...
    .reconnect-banner { position: fixed; top: 0; left: 0; right: 0; background: #f16876; color: #fff; text-align: center; font-size: 13px; font-weight: 500; padding: 6px; transform: translateY(-100%); transition: transform 0.3s ease; z-index: 9999; }
    .reconnect-banner.active { transform: translateY(0); }
  ...
  <div class="drawer-overlay" id="app-drawer-overlay" role="dialog" aria-modal="true" aria-labelledby="app-drawer-title">
    <aside class="app-drawer">
      ...
      <nav class="drawer-nav" aria-label="Main">
        ...
        <button class="drawer-nav-btn" data-drawer-destination="coder" type="button">
          <span>Coder</span><span class="drawer-nav-sub">Projects</span>
        </button>
  ...
  <div id="screen-home" class="screen">
    ...
    <div class="home-body" id="home-body">
      ...
      <div class="mode-toggle-row" id="mode-toggle-row">
        ...
        <button type="button" class="mode-toggle-btn" id="mode-toggle-coder" data-mode="coder">Coder</button>
      </div>
  ...
  <div id="screen-new-chat" class="screen">
    ...
    <div class="new-chat-body">
      <div class="dispatch-chat-intro" id="dispatch-chat-intro">
        <div class="dispatch-chat-title">Ask Orion anything</div>
        <div class="dispatch-chat-copy">Talk it through first. Bring in a project when it matters.</div>
      </div>
  ...
  <div class="dispatch-sheet-overlay" id="dispatch-browser-overlay" role="dialog" aria-modal="true" aria-labelledby="dispatch-browser-title">
    <section class="dispatch-sheet">
      ...
      <div class="dispatch-sheet-head">
        <div>
          <div class="dispatch-sheet-title" id="dispatch-browser-title">Browse Dispatch</div>
          <div class="dispatch-sheet-copy">Projects first, standalone discussions second.</div>
        </div>
    ...
    <div class="tab-panels">
      ...
      <div class="tab-panel active" id="panel-chat">
        <div class="messages" id="messages">
          <div class="dispatch-landing"><div class="dispatch-landing-intro"><div class="dispatch-landing-title">What do you need?</div><div class="dispatch-landing-copy">Start something new, or pick up a project.</div></div></div>
        </div>
...
<script>
  ...
  // On iOS/WebView, 100dvh does not shrink when the software keyboard opens,
  // so a bottom-anchored composer ends up hidden behind the keyboard. Track
  // the visual viewport instead and size .app-root to match; the existing
  // flex layout (messages: flex 1, composer: flex 0 0 auto) then keeps the
  // composer visible and lets the message list shrink/scroll around it.
  if (window.visualViewport) {
    const applyViewportHeight = () => {
      document.documentElement.style.setProperty('--app-vvh', window.visualViewport.height + 'px');
    };
  ...
  const modeToggleDispatchEl     = document.getElementById('mode-toggle-dispatch');
  const modeToggleCoderEl        = document.getElementById('mode-toggle-coder');
  ...
  // Dispatch (Orion) and Coder are separate entities with separate conversation histories on
  // mobile too -- this is a purely client-side view filter (which list the home screen shows),
  // independent of whatever conversation happens to be open. Every fresh app launch starts at
  // Orion's front door; choosing Coder still persists for navigation during this page session.
  ...
  function updateNewChatModeUI() {
    ...
    if (newChatScreenEl) {
      ...
      newChatScreenEl.classList.toggle('coder-mode', !isDispatchStart);
    }
    if (newChatPromptEl) {
      newChatPromptEl.placeholder = isDispatchStart ? 'Ask Orion anything...' : 'What should we build?';
    }
  }
  ...
  function updateDrawerState() {
    ...
    document.querySelectorAll('[data-drawer-destination]').forEach(btn => {
      ...
      else if (destination === 'coder') {
        active = currentScreen !== 'screen-settings' && currentScreen !== 'screen-home'
          ? activeMode === 'coder'
          : currentScreen === 'screen-home' && companionMode === 'coder';
      }
  ...
  function openModeHome(mode) {
    companionMode = mode === 'coder' ? 'coder' : 'orion';
  ...
  function startDispatchDraft(options = {}) {
    ...
    renderDispatchLanding(lastLoadedState || { conversations: allConversations, projects: availableProjects, activeWork: [] });
    updateDrawerState();
  ...
  async function enterDispatch(options = {}) {
    try {
      ...
      if (options.fresh === true || dispatchDraftActive) {
      ...
      '<div class="home-approval-list">' + scoped.map(c => {
        ...
        return '<div class="home-approval-card">' +
          '<div>' +
            ...
            '<div class="home-approval-meta">' + escapeHtml(projectName) + ' - ' + escapeHtml(conversationActivityLabel(c)) + '</div>' +
          '</div>' +
  ...
  function updateHomeScreen(state) {
    ...
    // Recent chats -- Dispatch and Coder each only ever see their own standalone chats here.
    ...
    homeRecentsTitleEl.textContent = companionMode === 'coder' ? 'Coder Chats' : 'Dispatch Discussions';
    ...
    if (filtered.length > 0) {
      ...
      homeRecentsEl.innerHTML = filtered.map(c => {
        ...
        else metaParts.push(conversationActivityLabel(c));
  ...
  function renderDispatchLanding(state) {
    if (!messagesEl) return;
    const projectGroups = buildDispatchProjectGroups().slice(0, 4);
    const activeWork = Array.isArray(state && state.activeWork) ? state.activeWork.slice(0, 4) : [];
    const selectedName = dispatchDraftProjectPath
      ? projectNameFromPath(dispatchDraftProjectPath, dispatchDraftProjectPath)
      : '';
    let html = '<div class="dispatch-landing">' +
      '<div class="dispatch-landing-intro">' +
        '<div class="dispatch-landing-title">What do you need?</div>' +
        '<div class="dispatch-landing-copy">Start something new, or pick up a project without reopening an old transcript.</div>' +
        (selectedName ? '<div class="dispatch-landing-context">Project context · ' + escapeHtml(selectedName) + '</div>' : '') +
      '</div>';

    if (activeWork.length) {
      html += '<section class="dispatch-landing-section"><div class="dispatch-landing-section-head"><span>Active work</span></div>' +
        activeWork.map(function(work) {
          const time = work.completedAt || work.startedAt;
          return '<button class="dispatch-work-row" type="button" data-open-active-work="' + escapeHtml(work.supervisingConversationId || '') + '">' +
            '<span class="dispatch-row-copy"><span class="dispatch-row-title">' + escapeHtml(work.title || 'Coder task') + '</span>' +
            '<span class="dispatch-row-meta">' + escapeHtml(work.projectName || 'Standalone') + ' · ' + escapeHtml(work.subStatus || work.status || '') + '</span></span>' +
            '<span><span class="dispatch-work-status ' + escapeHtml(work.status || '') + '">' + escapeHtml(work.status || 'working') + '</span>' +
            (time ? '<span class="dispatch-row-time"> · ' + relativeTime(time) + '</span>' : '') + '</span>' +
          '</button>';
        }).join('') + '</section>';
    }

    if (projectGroups.length) {
      html += '<section class="dispatch-landing-section"><div class="dispatch-landing-section-head"><span>Pick up a project</span>' +
        '<button class="dispatch-landing-browse" type="button" data-open-dispatch-browser="true">Browse</button></div>' +
        projectGroups.map(function(group) {
          const latest = group.latest;
          const summary = latest && latest.discussionSummary ? latest.discussionSummary : 'Start a new project discussion';
          const delegatedWork = activeWork.find(function(work) {
            return work.projectPath
              && String(work.projectPath).toLowerCase() === String(group.path).toLowerCase()
              && work.status !== 'completed';
          });
          return '<button class="dispatch-project-row" type="button" data-open-dispatch-project="' + escapeHtml(group.path) + '">' +
            '<span class="dispatch-row-copy"><span class="dispatch-row-title">' + escapeHtml(group.name) + '</span>' +
            '<span class="dispatch-row-meta">' + escapeHtml(summary) +
            (delegatedWork ? ' &middot; Coder ' + escapeHtml(delegatedWork.status || 'working') : '') + '</span></span>' +
            '<span class="dispatch-row-time">' + (group.updatedAt ? relativeTime(group.updatedAt) : '') + '</span>' +
          '</button>';
        }).join('') + '</section>';
    }

    if (!projectGroups.length) {
      html += '<section class="dispatch-landing-section"><button class="dispatch-landing-browse" type="button" data-open-dispatch-browser="true">Browse projects and discussions</button></section>';
    }
    html += '</div>';
    messagesEl.innerHTML = html;
    messagesEl.scrollTop = 0;
    chatProjNameEl.textContent = selectedName || 'Orion';
  ...
  function renderDispatchBrowser() {
    ...
    const groups = buildDispatchProjectGroups().map(function(group) {
      const searchable = [group.name].concat(group.conversations.map(function(conversation) {
        return (conversation.title || '') + ' ' + (conversation.discussionSummary || '');
      })).join(' ').toLowerCase();
      return { group: group, visible: !query || searchable.indexOf(query) !== -1 };
    }).filter(function(entry) { return entry.visible; }).map(function(entry) { return entry.group; });
    const standalone = allConversations.filter(function(conversation) {
      if ((conversation.mode || 'orion') !== 'orion' || conversation.dispatchProjectPath) return false;
      const searchable = ((conversation.title || '') + ' ' + (conversation.discussionSummary || '')).toLowerCase();
      return !query || searchable.indexOf(query) !== -1;
    }).sort(function(a, b) { return (b.updatedAt || 0) - (a.updatedAt || 0); });

    let html = '<div class="dispatch-browser-section-label">Projects</div>';
    html += groups.length ? groups.map(function(group) {
      const latest = group.latest;
      return '<details class="dispatch-browser-project">' +
        '<summary><span class="dispatch-row-copy"><span class="dispatch-row-title">' + escapeHtml(group.name) + '</span>' +
        '<span class="dispatch-row-meta">' + escapeHtml(latest && latest.discussionSummary || 'No Dispatch discussions yet') + '</span></span>' +
        '<span class="dispatch-row-time">' + group.conversations.length + '</span></summary>' +
        '<div class="dispatch-browser-project-threads">' +
          '<button class="dispatch-browser-thread" type="button" data-open-dispatch-project="' + escapeHtml(group.path) + '"><span>Project options</span><span aria-hidden="true">›</span></button>' +
        ...
        '</div></details>';
    }).join('') : '<div class="home-empty">No matching projects.</div>';

    html += '<div class="dispatch-browser-section-label">Standalone</div>';
    html += standalone.length ? standalone.map(function(conversation) {
      return '<button class="dispatch-browser-thread" type="button" data-open-dispatch-conversation="' + escapeHtml(conversation.id) + '">' +
        '<span class="dispatch-row-copy"><span class="dispatch-row-title">' + escapeHtml(conversation.title || 'Discussion') + '</span>' +
        '<span class="dispatch-row-meta">' + escapeHtml(conversation.discussionSummary || conversationActivityLabel(conversation)) + '</span></span>' +
        '<span class="dispatch-row-time">' + relativeTime(conversation.updatedAt) + '</span></button>';
    }).join('') : '<div class="home-empty">No standalone discussions.</div>';
  ...
  function conversationActivityLabel(conversation) {
    ...
    return parts.length ? parts.join(' + ') : 'No messages yet';
  }
  ...
  function renderProjectScreen() {
    ...
    projectThreadListEl.innerHTML = projectChats.map(c => {
      ...
      metaParts.push(conversationActivityLabel(c));
  ...
  function applyModeToggleUI() {
    modeToggleDispatchEl.classList.toggle('active', companionMode === 'orion');
    modeToggleCoderEl.classList.toggle('active', companionMode === 'coder');
    updateDrawerState();
  }
  applyModeToggleUI();
  ...
  messagesEl.addEventListener('click', event => {
    ...
    const activeWork = event.target.closest('[data-open-active-work]');
    if (activeWork) {
      const conversationId = activeWork.getAttribute('data-open-active-work') || '';
      if (conversationId) openConversation(conversationId);
    }
  });
  ...
  function setCompanionMode(mode) {
    const next = mode === 'coder' ? 'coder' : 'orion';
    if (next === 'orion') {
      enterDispatch();
      return;
    }
  ...
  modeToggleDispatchEl.addEventListener('click', () => setCompanionMode('orion'));
  modeToggleCoderEl.addEventListener('click', () => setCompanionMode('coder'));
  ...
  function openConversation(id) {
    ...
    if (target) {
      companionMode = (target.mode || 'orion') === 'coder' ? 'coder' : 'orion';
      localStorage.setItem('orionCompanionMode', companionMode);
      applyModeToggleUI();
      if (companionMode === 'orion') {
  ...
  function updateCoderTabVisibility(mode) {
    const isCoder = mode === 'coder';
    if (bottomNav) bottomNav.classList.toggle('hidden', !isCoder);
    if (!isCoder && !document.getElementById('panel-chat').classList.contains('active')) {
      switchTab('panel-chat');
    }
  }
  ...
  document.getElementById('main-back').addEventListener('click', () => {
    if (activeConversationMode === 'orion') openDispatchBrowser();
  ...
  async function startNewPhoneChat(projectPath, promptText) {
    // A project chat only ever belongs to Coder; a standalone chat inherits whichever mode
    // (Dispatch/Coder) is currently selected on the home screen toggle.
    ...
    const res = await companionFetch('/api/conversations/new', {
      method: 'POST',
      body: JSON.stringify({ prompt: promptText || '', projectPath: projectPath || '', mode })
    });
    const data = await res.json();
    ...
    showScreen('screen-main');
    if (promptText && mode !== 'orion') {
      const pRes = await companionFetch('/api/prompt', {
        method: 'POST',
        body: JSON.stringify({ prompt: promptText, projectPath: projectPath || '', mode })
      });
      const pData = await pRes.json();
      if (!pData.success) throw new Error(pData.error || 'Send failed');
    }
  }
  ...
  function recoverIdleAssistantPlaceholder(text, hasActivity, isRunning) {
    ...
    return 'Run ended before Orion saved an assistant response.';
  }
  ...
  async function loadPhoneModelList() {
    ...
    try {
      ...
      // Rebuild option list grouped by provider
      ...
      Object.entries(groups).forEach(([groupLabel, models]) => {
        ...
        phoneModelSelect.appendChild(optgroup);
      });
      if (current) phoneModelSelect.value = current;
    } catch (e) {
  ...
  if (phoneModelSelect) {
    phoneModelSelect.addEventListener('focus', () => { phoneModelSelect._userChanging = true; });
    phoneModelSelect.addEventListener('blur', () => { phoneModelSelect._userChanging = false; });
    phoneModelSelect.addEventListener('change', async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (!data.success) {
          statusEl.textContent = 'Model change failed: ' + (data.error || 'unknown error');
          await loadPhoneModelList(); // revert
        }
      } catch (e) {
        statusEl.textContent = e.message;
      } finally {
        phoneModelSelect.disabled = false;
        phoneModelSelect._userChanging = false;
      }
    });
    // Load model list once on boot
    loadPhoneModelList();
  }
  ...
  function renderDispatchEmptyState() {
    return '<div class="dispatch-empty-chat">' +
      '<div class="dispatch-empty-title">What do you need?</div>' +
      '<div class="dispatch-empty-status">Orion is ready.</div>' +
      '<div class="dispatch-empty-actions">' +
        ...
        '<button class="dispatch-empty-action" type="button" data-dispatch-prompt="Look over one of my projects with me.">Open a project</button>' +
  ...
  function renderConversationMessages(state) {
    ...
    const renderedMessages = messages.map(msg => {
          ...
          // Render any attached images (user messages with vision input)
          const imagesHtml = (msg.role === 'user' && Array.isArray(msg.images) && msg.images.length > 0)
            ? msg.images.map(img => img && img.data && img.mimeType
                ? '<img class="message-image" src="data:' + escapeHtml(img.mimeType) + ';base64,' + img.data + '" alt="attached image">'
                : '').join('')
            : '';
          ...
          return '<div class="message ' + escapeHtml(msg.role) + (hasActivity ? ' has-activity' : '') + '"><span class="role">' + escapeHtml(msg.role) + '</span>' +
            logsHtml + walkthroughHtml + bodyHtml + '</div>';
        }).filter(Boolean).join('');
  ...
  async function loadState(options = {}) {
    ...
    // The 3-second poll fires every tick regardless of whether it does anything; if SSE has
    // delivered a message recently the poll has no work to do, but if we increment stateRequestSerial
    // first the bump permanently invalidates any concurrent switchTask's minSerial check.
    if (!options.force && Date.now() - lastSseMessageAt < 2500) return;
  ...
  function applyState(state) {
      ...
      updateCoderTabVisibility(preserveDispatchDraft ? 'orion' : (state.mode || 'orion'));
      // Sync phone model select to desktop's current model (don't override if user is mid-change)
      if (phoneModelSelect && state.model && !phoneModelSelect._userChanging) {
        if (!Array.from(phoneModelSelect.options).some(o => o.value === state.model)) {
        ...
        phoneModelSelect.value = state.model;
      }
      ...
      const coderTaskStillOwned = runningId && (state.conversations || []).some(c => c.launchedCoderConvId === runningId);
      const showBackgroundCoder = !preserveDispatchDraft && activeConversationMode === 'orion' && globalRunning && runningId !== viewingId
        && runningTaskObj && (runningTaskObj.mode || 'orion') === 'coder' && coderTaskStillOwned;
      dispatchRunningBanner.classList.toggle('visible', !!showBackgroundCoder);
      if (showBackgroundCoder) {
        dispatchRunningBanner.setAttribute('data-conversation-id', runningId);
        dispatchRunningText.textContent = 'Coder working on ' + (runningTaskObj.title || 'a task');
      } else {
      ...
      renderPhoneTaskList(state.tasks || []);
      ...
      if (modeScopedTasks.length > 0) {
        recentTasksList.innerHTML = modeScopedTasks.map(c => {
          ...
          return '<div class="task-row' + (isViewing ? ' active-row' : '') + '" data-switch-task="' + escapeHtml(c.id) + '">' +
            ...
            '<div class="task-row-meta">Updated: ' + timeText + ' &middot; ' + escapeHtml(conversationActivityLabel(c)) + '</div>' +
            '</div>' +
      ...
      if (!initialScreenResolved) {
        ...
        if (companionMode === 'orion') setTimeout(() => startDispatchDraft(), 0);
      }
  }
  ...
  async function handlePromptSubmit() {
    ...
    try {
      if (formMode === 'steer') {
        ...
        const promptPayload = { prompt: text };
        if (phoneImageData) {
  ...
  async function startEventStream() {
    if (!deviceSession) {
      setTimeout(startEventStream, 800);
      return;
    }
    if (sseActive) return;
    sseActive = true;
  ...
  async function pairIfNeeded() {
    ...
    const code = urlPairingCode || pairingCode;
    ...
    try {
      ...
      if (!data.success && urlPairingCode && urlPairingCode !== pairingCode) {
  ...
  async function subscribePush(reg) {
    try {
      ...
      if (!publicKey) {
        setNotificationBanner('Phone push is unavailable: notification keys are missing.', 'Retry', true);
        return;
      }
