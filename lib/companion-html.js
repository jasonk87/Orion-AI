...
<script>
  ...
  window.onerror = function(msg, src, line) {
    ...
    if (el) el.textContent = 'ERR ' + line + ': ' + msg;
  };
  ...
  // ── Debug tap test (remove after confirming clicks work) ──
  const dbgTapBtn = document.getElementById('__dbg-tap');
  if (dbgTapBtn) {
    dbgTapBtn.addEventListener('click', function() {
      this.textContent = 'WORKS ✓ (listener)';
      this.style.background = '#10b981';
      var dbg = document.getElementById('__dbg');
      if (dbg) { dbg.style.display = 'block'; dbg.textContent = 'addEventListener WORKS ✓'; dbg.style.background = '#10b981'; }
    });
  }
...
<!-- Debug overlay: error display + tap test button -->
<div id="__dbg" style="display:none;position:fixed;top:0;left:0;right:0;background:#ef4444;color:#fff;padding:8px 12px;font-size:13px;font-weight:700;z-index:99999;word-break:break-all;" onclick="this.style.display='none'"></div>
<button id="__dbg-tap" type="button" onclick="(function(b){b.textContent='ONCLICK ✓';b.style.background='#10b981';var d=document.getElementById('__dbg');d.style.display='block';d.textContent='onclick FIRED ✓ - tap again to test listener';d.style.background='#059669';})(this)" style="position:fixed;bottom:80px;right:12px;z-index:99999;background:#ef4444;color:#fff;border:none;border-radius:10px;padding:14px 18px;font-size:15px;font-weight:800;box-shadow:0 4px 20px rgba(0,0,0,0.5);">TAP TEST</button>
</body>
</html>`;
}
