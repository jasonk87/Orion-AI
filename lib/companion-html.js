...
// `scriptNonce` is supplied fresh per HTTP response by lib/ipc-server.js and stamped on the one
// inline <script> below. A nonce rather than a hash because this document is templated per request,
// so a hash would have to be recomputed for every response anyway — and a nonce keeps script-src
// strict without ever needing 'unsafe-inline'.
function companionHtml(machineName, scriptNonce = '') {
  // Only base64/URL-safe characters survive, so a nonce can never break out of the attribute.
  const nonceAttribute = scriptNonce
    ? ` nonce="${String(scriptNonce).replace(/[^A-Za-z0-9+/=_-]/g, '')}"`
    : '';
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Real bug found from screenshots: a normal <table> grid at phone width forces narrow columns
       to wrap a single word into a vertical letter-stack ("Wh/ere", "Add-/on"). CSS alone cannot fix
       this (there is no room to reflow a 3-4 column grid narrower than a word), so the phone
       companion's markdown renderer (renderMarkdown/renderMarkdownFallback below) restructures every
       table into stacked cards instead of emitting <table> markup at all — one card per row, the
       first column as a bold title, every other column as a "Label: value" line. This is scoped to
       this file (the phone companion's own client script) on purpose: renderer.js (desktop) has its
       own separate marked.parse() call and is untouched, so desktop keeps normal grid tables.
       .message-answer table/th/td above intentionally remain as a defensive fallback in case any raw
       HTML table ever reaches this container outside the markdown path. */
    .md-table-cards { display: flex; flex-direction: column; gap: 8px; margin: 10px 0; }
    .md-table-card { border: 1px solid var(--line); border-radius: 8px; padding: 9px 11px; background: var(--surface); }
    .md-table-card-title { font-weight: 700; margin-bottom: 4px; word-break: break-word; }
    .md-table-card-row { display: flex; flex-wrap: wrap; gap: 4px 6px; padding: 3px 0; font-size: 0.92em; border-top: 1px solid rgba(255,255,255,0.06); }
    .md-table-card-row:first-of-type { border-top: none; }
    .md-table-card-label { color: var(--muted); }
    .md-table-card-value { word-break: break-word; }
...
<div id="__dbg" style="display:none;position:fixed;top:0;left:0;right:0;z-index:99999;background:#7f1d1d;color:#fff;padding:8px 12px;font-size:12px;font-weight:700;word-break:break-word;"></div>

<script${nonceAttribute}>
  // ── Diagnostics (remove after debugging) ─────────
  window.__dbgErrors = [];
  // Dismiss-on-tap for the diagnostics banner, previously an inline onclick attribute. Inline
  // handlers count as script for CSP, so keeping one would have forced script-src to allow
  // unsafe-inline - which is most of what the policy exists to prevent on a page that renders
  // model-authored markdown.
  (function () {
    var dbgEl = document.getElementById('__dbg');
    if (dbgEl) dbgEl.addEventListener('click', function () { dbgEl.style.display = 'none'; });
  })();
  ...
  // Previous offset, so the scroll handler can tell a deliberate move UP from content growing.
  let lastScrollTop = 0;

  // Whether the transcript keeps following the newest message. ONLY a deliberate move upward stops
  // it following.
  //
  // This used to be recomputed purely from geometry on every scroll event, which meant anything
  // that grew the content unpinned the reader without them touching the screen. Send a message:
  // the transcript pins to the bottom, then image hydration and markdown layout grow the list, a
  // scroll event fires with a large distance-from-bottom, and following silently switches off. The
  // next state render then took the "restore the previous offset" branch and jumped the view back
  // UP to the last assistant message - past the user's own message and the thinking bubble they
  // were watching. Content growing below the reader is not the reader scrolling away from it.
  //
  // Downward movement and growth can only ever RE-pin, never unpin, so the transcript also keeps
  // following while an answer streams in underneath.
  function resolveTranscriptFollowing(wasFollowing, previousScrollTop, currentScrollTop, distanceFromBottom) {
    // One pixel of slack: sub-pixel offsets and rounding must not read as a deliberate scroll up.
    const scrolledUp = currentScrollTop < previousScrollTop - 1;
    if (scrolledUp) return distanceFromBottom < NEAR_BOTTOM_PX;
  ...
  if (messagesEl) {
    messagesEl.addEventListener('scroll', () => {
      ...
      userPinnedToBottom = resolveTranscriptFollowing(
        userPinnedToBottom,
        lastScrollTop,
        currentScrollTop,
        distanceFromBottom
  ...
  // One row of a real markdown table becomes one card: the first column is the bold title (not a
  // "Label: value" line — it is the thing the row is about), every remaining column becomes a
  // "Label: value" line using that column's header as the label. Takes already-rendered inline HTML
  // (bold/links/code survive), never raw text, so this matches what a normal <table> cell would show.
  function buildMarkdownTableCardHtml(headerCellsHtml, rowCellsHtml) {
    const titleHtml = rowCellsHtml[0] || '';
    const rows = rowCellsHtml.slice(1).map((valueHtml, index) => {
      const labelHtml = headerCellsHtml[index + 1] || '';
      return '<div class="md-table-card-row"><span class="md-table-card-label">' + labelHtml
        + '</span><span class="md-table-card-value">' + valueHtml + '</span></div>';
    }).join('');
    return '<div class="md-table-card"><div class="md-table-card-title">' + titleHtml + '</div>' + rows + '</div>';
  }

  // Real bug found from phone screenshots: a normal grid <table> at phone width forces narrow
  // columns to wrap a single word into a vertical letter-stack. marked.parse() (the primary
  // renderer below) still produces ordinary <table> markup — it has no idea it is rendering for a
  // narrow phone screen — so every <table> it produces is walked here and replaced with the same
  // stacked-card markup buildMarkdownTableCardHtml produces for the fallback parser, keeping both
  // markdown code paths in this file visually identical. This only ever runs on this file's own
  // rendered output (the phone companion's client script); renderer.js (desktop) has its own
  // separate marked.parse() call with no such transform, so desktop keeps normal grid tables.
  function restructureMarkdownTablesAsCards(root) {
    root.querySelectorAll('table').forEach(table => {
      const headerCellsHtml = Array.from(table.querySelectorAll('thead th')).map(th => th.innerHTML);
      const cardsHtml = Array.from(table.querySelectorAll('tbody tr')).map(tr => {
        const rowCellsHtml = Array.from(tr.children).map(cell => cell.innerHTML);
        return buildMarkdownTableCardHtml(headerCellsHtml, rowCellsHtml);
      }).join('');
      const wrapper = document.createElement('div');
      wrapper.className = 'md-table-cards';
  ...
  function sanitizeMarkdownHtml(html) {
    ...
    restructureMarkdownTablesAsCards(template.content);
  ...
  function consumeMarkdownTable(lines, startIndex) {
    ...
    // Same restructure as the marked.parse() path (see restructureMarkdownTablesAsCards above):
    // stacked cards instead of a <table> grid, so a narrow phone width never forces a column into
    // a single-word letter-stack. Cell text is rendered through renderInlineMarkdown first so bold,
    // links, and code spans survive, matching what the old <th>/<td> markup used to preserve.
    const headerCellsHtml = headers.map(cell => renderInlineMarkdown(cell));
    const cardsHtml = bodyRows
      .map(row => buildMarkdownTableCardHtml(headerCellsHtml, row.map(cell => renderInlineMarkdown(cell))))
      .join('');
    return { html: '<div class="md-table-cards">' + cardsHtml + '</div>', nextIndex: index };
  }
  ...
  const chatImageFetches = new Map();
  const MAX_CHAT_IMAGE_OBJECT_URLS = 160;

  function buildChatImageCacheKey(image, containingConversationId, messageVersion) {
    return [
      containingConversationId,
      image && image.sourceConversationId,
      image && image.path,
      image && (image.cacheKey || image.hash || image.updatedAt),
      messageVersion
    ].map(value => encodeURIComponent(String(value || ''))).join('|');
  }

  function getChatImageObjectUrl(cacheKey) {
    const cached = chatImageObjectUrls.get(cacheKey);
    if (!cached) return '';
    chatImageObjectUrls.delete(cacheKey);
    chatImageObjectUrls.set(cacheKey, cached);
    return cached;
  }

  function pruneChatImageObjectUrls() {
    if (chatImageObjectUrls.size <= MAX_CHAT_IMAGE_OBJECT_URLS) return;
    const visibleKeys = new Set(Array.from(messagesEl.querySelectorAll('img[data-chat-image-cache-key]'))
      .map(image => image.getAttribute('data-chat-image-cache-key') || '')
      .filter(Boolean));
    for (const [cacheKey, objectUrl] of chatImageObjectUrls) {
      if (chatImageObjectUrls.size <= MAX_CHAT_IMAGE_OBJECT_URLS) break;
      // Do not revoke a URL that a currently visible image is still rendering. The phone state
      // transports at most 160 referenced images, so the cache remains bounded while older,
      // non-visible conversations are evicted first.
      if (visibleKeys.has(cacheKey)) continue;
      URL.revokeObjectURL(objectUrl);
      chatImageObjectUrls.delete(cacheKey);
    }
  }

  function rememberChatImageObjectUrl(cacheKey, objectUrl) {
    const previous = chatImageObjectUrls.get(cacheKey);
    if (previous && previous !== objectUrl) URL.revokeObjectURL(previous);
    chatImageObjectUrls.delete(cacheKey);
    chatImageObjectUrls.set(cacheKey, objectUrl);
    pruneChatImageObjectUrls();
  }

  function forgetChatImageObjectUrl(cacheKey, objectUrl) {
    const current = chatImageObjectUrls.get(cacheKey);
    if (!current || (objectUrl && current !== objectUrl)) return;
  ...
  function releaseChatImageObjectUrls() {
    ...
    chatImageObjectUrls.clear();
    chatImageFetches.clear();
  }

  function renderMessageImages(images, defaultConversationId, messageVersion) {
    return (Array.isArray(images) ? images : []).slice(0, 4).map(img => {
      if (!img) return '';
      // The containing transcript authorizes this attachment. sourceConversationId is retained
      // as provenance for renderer.js to read a relayed Operator/Coder artifact, but that source
      // conversation does not necessarily contain the relayed image message itself.
      const conversationId = defaultConversationId || img.sourceConversationId || '';
      const cacheKey = buildChatImageCacheKey(img, conversationId, messageVersion);
      ...
      if (!img.path) return '';
      return '<figure class="message-image-figure"><img class="message-image" data-chat-image-path="' + escapeHtml(img.path) + '" data-chat-image-conversation="' + escapeHtml(conversationId || '') + '" data-chat-image-cache-key="' + escapeHtml(cacheKey) + '" alt="' + alt + '"><span class="message-image-status">Loading image…</span>' + caption + '</figure>';
    }).join('');
  }

  async function fetchChatImageObjectUrl(cacheKey, conversationId, path) {
    const cached = getChatImageObjectUrl(cacheKey);
    if (cached) return cached;
    const existingFetch = chatImageFetches.get(cacheKey);
    ...
    const pendingFetch = (async () => {
      ...
      rememberChatImageObjectUrl(cacheKey, objectUrl);
      return objectUrl;
    })();
    chatImageFetches.set(cacheKey, pendingFetch);
    try {
      return await pendingFetch;
    } finally {
      if (chatImageFetches.get(cacheKey) === pendingFetch) chatImageFetches.delete(cacheKey);
    }
  }

  function displayMessageImage(image, objectUrl) {
    return new Promise((resolve, reject) => {
      ...
      const onLoad = () => {
        ...
        else finish(new Error('Image decoded with no visible dimensions'));
      };
      const onError = () => finish(new Error('Image could not be decoded or displayed'));
      const timeout = window.setTimeout(
        () => finish(new Error('Image display timed out')),
        15000
      );
      image.addEventListener('load', onLoad);
      image.addEventListener('error', onError);
      image.src = objectUrl;
      // A page-lifetime cached object URL may already be decoded before listeners are attached
      // on some mobile Chromium builds. Resolve that path from the element's real dimensions,
      // never merely from src being present.
      if (image.complete) window.setTimeout(onLoad, 0);
    });
  }
  ...
  async function loadMessageImage(image, attempt = 0) {
      ...
      const cacheKey = image.getAttribute('data-chat-image-cache-key') || buildChatImageCacheKey({ path }, conversationId, '');
      let objectUrl = '';
      try {
        objectUrl = await fetchChatImageObjectUrl(cacheKey, conversationId, path);
        if (!image.isConnected) return;
        await displayMessageImage(image, objectUrl);
        if (!image.isConnected) return;
        image.dataset.loading = 'false';
        if (status) status.remove();
      } catch (_) {
        // A successful HTTP fetch is not a successful image. Decode/CSP failures invalidate the
        // page-local display URL so the bounded retry performs a real refetch instead of reusing
        // the same broken value and falsely declaring the attachment loaded.
        if (objectUrl) forgetChatImageObjectUrl(cacheKey, objectUrl);
  ...
  // State refreshes must not destroy images that the next render immediately needs again. The
  // bounded page-lifetime cache is released only when the phone page itself exits.
  window.addEventListener('beforeunload', releaseChatImageObjectUrls);
  ...
  function renderConversationMessages(state) {
    ...
    const renderedMessages = dedupedMessages.map(msg => {
          ...
          const imagesHtml = renderMessageImages(msg.images, state.conversationId, msg.id || msg.requestId || msg.createdAt || '');
      ...
      if (!deviceSession) {
        ...
        if (!pairResult.success) {
          ...
          if (pairResult.needsPairingLink) {
            ...
            // No inline onclick: inline handlers are script under CSP, and allowing them would mean
            // allowing every injected handler too. Wired up right after insertion instead.
            statusEl.innerHTML = 'Pairing denied. <button class="btn-sm" id="pair-retry-btn">Retry</button>';
            var retryBtn = document.getElementById('pair-retry-btn');
  ...
  function appendOptimisticPhoneMessage(text, images, requestId) {
    ...
    const imagesHtml = renderMessageImages(images, currentConversationId, requestId || 'optimistic');
