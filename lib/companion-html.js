...
<script>
  ...
  async function loadPhoneModelList() {
    ...
    try {
      ...
      fillModelOptions(phoneModelSelect, data.models, current);
      fillModelOptions(composerModelSelect, data.models, current);
      ...
      reflectReasoningSelection(data.reasoning || 'auto');
    } catch (e) {
  ...
  function wirePhoneModelSelect(select, otherSelect) {
    if (!select) return;
    select.addEventListener('focus', () => { select._userChanging = true; });
    select.addEventListener('blur', () => { select._userChanging = false; });
    select.addEventListener('change', async () => {
      ...
      try {
        ...
        if (data.success && otherSelect
            && Array.from(otherSelect.options).some(o => o.value === model)) {
          otherSelect.value = model;
        }
      } finally {
        select.disabled = false;
        select._userChanging = false;
      }
    });
  }
  ...
  if (composerReasoningSelect) {
    composerReasoningSelect.addEventListener('change', async () => {
      ...
      try {
        ...
        reflectReasoningSelection(data.success ? (data.reasoning || reasoning) : 'auto');
      } finally {
  ...
  function applyState(state) {
      ...
      // Sync phone model selects to desktop's current model (don't override if user is mid-change)
      [phoneModelSelect, composerModelSelect].forEach(select => {
        if (!select || !state.model || select._userChanging) return;
      ...
      // A reasoning level picked on the desktop must show up here too.
      if (state.reasoning) reflectReasoningSelection(state.reasoning);
