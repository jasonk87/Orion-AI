...
const COMPANION_CLIENT_BUILD = 'msg-render-reset-v5';
...
<script>
  ...
  function renderProjectScreen() {
    ...
    projectThreadListEl.innerHTML = projectChats.map(c => {
      ...
      metaParts.push((c.taskCount || 0) + ' items');
  ...
  async function deleteHomeConversation(id) {
    if (!id || !confirm('Delete this chat from Orion?')) return;
  ...
  function renderConversationMessages(state) {
    ...
    messagesEl.innerHTML = messages.length === 0
      ...
      : messages.map(msg => {
          ...
          return '<div class="message ' + escapeHtml(msg.role) + '"><span class="role">' + escapeHtml(msg.role) + '</span>' +
            (msg.role === 'system' ? escapeHtml(messageText) : renderMarkdown(messageText)) + '</div>';
        }).join('');
