...
<script>
  ...
  function updateHomeScreen(state) {
    ...
    homeRecentsTitleEl.textContent = 'Recent Chats';
  ...
  async function startNewPhoneChat(projectPath, promptText) {
    ...
    if (promptText) {
      const pRes = await companionFetch('/api/prompt', {
        method: 'POST',
        body: JSON.stringify({ prompt: promptText, projectPath: projectPath || '' })
      });
  ...
  sheetStart.addEventListener('click', async () => {
    ...
    try {
      ...
      if (initialPrompt) {
        const pRes = await companionFetch('/api/prompt', { method: 'POST', body: JSON.stringify({ prompt: initialPrompt, projectPath: selectedSheetProject }) });
        const pData = await pRes.json();
