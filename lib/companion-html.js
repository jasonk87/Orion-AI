...
const COMPANION_CLIENT_BUILD = 'responsive-phone-state-v29';
...
<script>
  ...
  async function loadState(options = {}) {
    ...
    if (!options.force && ((sseConnected && Date.now() - lastSseActivityAt < 25000) || stateFetchController)) return;
  ...
  startEventStream();
  // Give the stream handshake a brief head start. The server sends its connection frame before
  // computing the full snapshot, so a healthy stream suppresses the redundant HTTP state load.
  setTimeout(() => {
    if (!sseConnected && !sseStateReceived) loadState();
  }, 900);
  ...
  function recoverForegroundConnection() {
    ...
    foregroundRecoveryTimer = setTimeout(() => {
      ...
      setTimeout(() => {
        if (!sseConnected && !stateFetchController) loadState();
      }, 900);
      setTimeout(() => {
        if (!sseConnected && lastSseMessageAt < recoveryStartedAt && !stateFetchController) loadState({ force: true });
      }, 4500);
      setTimeout(() => {
        if (!sseConnected && lastSseMessageAt < recoveryStartedAt) showReconnectBanner();
      }, 650);
    }, 50);
  }
