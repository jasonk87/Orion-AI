...
const COMPANION_CLIENT_BUILD = 'dispatch-front-door-v19';
  ...
  <div class="drawer-overlay" id="app-drawer-overlay" role="dialog" aria-modal="true" aria-labelledby="app-drawer-title">
    <aside class="app-drawer">
      ...
      <nav class="drawer-nav" aria-label="Main">
        ...
        <button class="drawer-nav-btn" data-drawer-destination="history" type="button">
          <span>History</span><span class="drawer-nav-sub">Dispatch topics</span>
        </button>
...
<script>
  ...
  const dispatchFocusKey = 'orionLastDispatchFocus';
  ...
  function updateDrawerState() {
    ...
    document.querySelectorAll('[data-drawer-destination]').forEach(btn => {
      ...
      else if (destination === 'history') active = currentScreen === 'screen-home' && companionMode === 'orion';
  ...
  function rememberDispatchFocus(conversationId) {
    if (!conversationId) return;
    localStorage.setItem(dispatchFocusKey, conversationId);
  }

  function resolveDispatchFocus(state = lastLoadedState) {
    const dispatchConversations = ((state && state.conversations) || allConversations || [])
      .filter(conversation => (conversation.mode || 'orion') === 'orion');
    if (!dispatchConversations.length) return null;
    const byId = id => dispatchConversations.find(conversation => conversation.id === id);
    const running = state && state.globalRunning ? byId(state.runningConversationId) : null;
    const selected = activeConversationMode === 'orion' ? byId(currentConversationId) : null;
    const remembered = byId(localStorage.getItem(dispatchFocusKey) || '');
    return running || selected || remembered || dispatchConversations
      .slice()
  ...
  async function enterDispatch(options = {}) {
    try {
      ...
      if (options.fresh === true) {
        await startNewPhoneChat('', '');
        return;
      }

      const target = resolveDispatchFocus(options.state || lastLoadedState);
      if (!target) {
        await startNewPhoneChat('', '');
        return;
      }

      rememberDispatchFocus(target.id);
  ...
  function renderApprovalCards(conversations, options = {}) {
    ...
    return '<div class="home-section" data-approval-section="true">' +
      ...
      '<div class="home-approval-list">' + scoped.map(c => {
        const projectName = c.projectPath ? c.projectPath.split(/[\\/]/).pop() || c.projectPath : 'Standalone';
  ...
  function updateHomeScreen(state) {
    ...
    homeRecentsTitleEl.textContent = companionMode === 'coder' ? 'Coder Chats' : 'Dispatch History';
  ...
  function openConversation(id) {
    ...
    if (target) {
      ...
      localStorage.setItem('orionCompanionMode', companionMode);
      applyModeToggleUI();
      if (companionMode === 'orion') rememberDispatchFocus(id);
    }
    lastSignature = '';
    messagesEl.innerHTML = '<div class="empty">Loading conversation...</div>';
    showScreen('screen-main');
  ...
  document.getElementById('main-back').addEventListener('click', () => {
    if (activeConversationMode === 'orion') openDispatchHistory();
  ...
  async function startNewPhoneChat(projectPath, promptText) {
    ...
    if (mode === 'orion') rememberDispatchFocus(currentConversationId);
  ...
  function applyState(state) {
      ...
      if (activeConversationMode === 'orion') rememberDispatchFocus(currentConversationId);
      if (newFocusChipEl) {
      ...
      updateCoderTabVisibility(state.mode || 'orion');
      ...
      const showBackgroundCoder = activeConversationMode === 'orion' && globalRunning && runningId !== viewingId
      ...
      if (!initialScreenResolved) {
        ...
        if (companionMode === 'orion') setTimeout(() => enterDispatch({ state }), 0);
      }
  }
