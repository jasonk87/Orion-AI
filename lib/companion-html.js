...
<script>
  ...
  document.addEventListener('click', event => {
    ...
    const projectRow = target.closest('[data-open-project]');
    if (projectRow) {
      openProjectInNewChat(projectRow.getAttribute('data-open-project') || '', projectRow.getAttribute('data-project-name') || '');
      return;
    }
    const conversationRow = target.closest('[data-open-conversation]');
    if (conversationRow) {
      openConversation(conversationRow.getAttribute('data-open-conversation') || '');
      return;
    }
