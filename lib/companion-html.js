...
<script>
  ...
  function renderDispatchLanding(state) {
    ...
    if (recentSessions.length) {
      html += '<section class="dispatch-landing-section"><div class="dispatch-landing-section-head"><span>Recent</span></div>' +
        recentSessions.map(function(conversation) {
          const projectPath = conversation.dispatchProjectPath || conversation.projectPath || '';
          const projectName = projectPath ? projectNameFromPath(projectPath, projectPath) : '';
          return '<button class="dispatch-project-row" type="button" data-open-recent-session="' + escapeHtml(conversation.id || '') + '">' +
            ...
            (projectName ? '<span class="dispatch-row-meta">' + escapeHtml(projectName) + '</span>' : '') + '</span>' +
