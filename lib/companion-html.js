...
const COMPANION_CLIENT_BUILD = 'durable-phone-auth-v23';
...
<script>
  ...
  let acceptedSelectionRevision = 0;
  ...
  async function startNewPhoneChat(projectPath, promptText) {
    ...
    acceptedSelectionRevision = Math.max(acceptedSelectionRevision, Number(data.selectionRevision) || 0);
  ...
  function applyState(state) {
      ...
      if (stateSelectionRevision < acceptedSelectionRevision) return;
      ...
      acceptedSelectionRevision = Math.max(acceptedSelectionRevision, stateSelectionRevision);
      ...
      const supervisedPresentation = supervisedTask
        ...
        : null;
      const phoneAgentState = supervisedPresentation
        ? supervisedPresentation.agentState
      ...
      statusPillEl.classList.toggle('running', supervisedPresentation
        ? !!supervisedPresentation.isOngoing
        : !!state.running);
      statusEl.textContent = supervisedPresentation
      ...
      if (showBackgroundCoder) {
        ...
        dispatchRunningText.textContent = supervisedPresentation.label + ': ' + (supervisedTask.title || 'Coder task');
      } else {
      ...
      if (activeConv) {
        ...
        const isRunning = hasSupervisedTask
          ? supervisedPresentation.isOngoing
          : (globalRunning && runningId === viewingId);
        const statusText = hasSupervisedTask
          ? supervisedPresentation.label
          : (isRunning ? 'Running' : (activeConv.awaitingPlanApproval ? 'Needs Attention' : 'Idle'));
        const badgeClass = hasSupervisedTask
        ...
        activeTaskContainer.innerHTML =
          ...
          '</div>' +
          '<div class="card-title">' + escapeHtml(hasSupervisedTask ? supervisedTask.title : activeConv.title) + '</div>' +
          '<div class="substatus-text">' + escapeHtml(hasSupervisedTask ? supervisedPresentation.detail : (state.subStatus || state.workspace || '')) + '</div>';
  ...
  async function pairIfNeeded() {
    ...
    if (!urlPairingCode) {
      statusEl.textContent = 'This browser has no saved Orion phone access. Open a new pairing link once.';
      return { success: false, pending: false, needsPairingLink: true };
    }
