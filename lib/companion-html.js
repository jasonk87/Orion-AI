...
const COMPANION_CLIENT_BUILD = 'ui-polish-v13';
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .clarification-panel { border-color: rgba(96,165,250,0.28); background: rgba(96,165,250,0.05); }
    ...
    /* Desktop-aligned Orion mobile refinement */
    :root {
      color-scheme: dark;
      --bg: #090b12;
      --surface: #111827;
    ...
    .mark, button.send-button, .sheet-start-btn, .approve-button {
      background: linear-gradient(135deg, #8273f4, #4f8dff);
  ...
  <div id="screen-main" class="screen">
    ...
    <div class="tab-panels">
      ...
      <div class="tab-panel" id="panel-status">
        ...
        <section class="plan-panel clarification-panel" id="clarification-panel">
          <div id="clarification-intro" class="clarification-intro"></div>
          <div id="clarification-questions"></div>
          <div class="clarification-actions">
            <button class="btn-clarification-submit" id="clarification-submit" type="button">Submit</button>
          </div>
        </section>
...
<script>
  ...
  const clarificationIntroEl     = document.getElementById('clarification-intro');
  const clarificationQuestionsEl = document.getElementById('clarification-questions');
  const clarificationSubmitEl    = document.getElementById('clarification-submit');
  let lastClarificationSignature = '';
  ...
  function renderInlineMarkdown(text) {
    ...
    const placeholders = [];
    let html = escapeHtml(text).replace(codePattern, function(match, code) {
  ...
  function renderWorkWalkthroughBlock(walkthroughText) {
    ...
    return '<div class="agent-logs-container walkthrough-panel">' +
      '<div class="agent-logs-header"><span>Work Walkthrough (' + rows.length + ' items)</span><span>open</span></div>' +
  ...
  // Mirrors the desktop's buildClarificationCardHtml (renderer.js): same field names
  // (intro, questions[].header/question/options[].label/description/recommended) plus a
  // synthetic "Other" free-text option per question. Only re-renders when the underlying
  // question data actually changes, so a user's in-progress selections survive the ~1-2s poll tick.
  function renderClarificationPanel(clarData) {
    const signature = clarData ? JSON.stringify(clarData) : '';
    clarificationPanelEl.classList.toggle('visible', !!clarData);
    if (!clarData || signature === lastClarificationSignature) return;
    lastClarificationSignature = signature;

    clarificationIntroEl.textContent = clarData.intro || '';
    const questions = Array.isArray(clarData.questions) ? clarData.questions : [];
    clarificationQuestionsEl.innerHTML = questions.map((q, qi) => {
    ...
    clarificationSubmitEl.disabled = false;
    clarificationSubmitEl.textContent = 'Submit';
  }

  clarificationSubmitEl.addEventListener('click', async () => {
    ...
    questions.forEach((q, qi) => {
      const block = clarificationQuestionsEl.querySelector('.clarification-question-block[data-qi="' + qi + '"]');
      let answer = null;
      if (block) {
    ...
    if (!allAnswered) {
      const orig = clarificationSubmitEl.textContent;
      clarificationSubmitEl.textContent = 'Answer all questions first';
      setTimeout(() => { clarificationSubmitEl.textContent = orig; }, 1800);
      return;
    }

    clarificationSubmitEl.disabled = true;
    clarificationSubmitEl.textContent = 'Submitting…';
    try {
  ...
  function renderConversationMessages(state) {
    ...
    messagesEl.innerHTML = (renderedMessages || typingHtml)
      ? renderedMessages + typingHtml
  ...
  function renderToolCallRows(toolCalls) {
    ...
    const rows = toolCalls.slice(-8).map(call => {
      ...
      const params = call.params && typeof call.params === 'object' ? JSON.stringify(call.params) : '{}';
      const result = call.result ? '<div class="tool-result-box">' + escapeHtml(call.result) + '</div>' : '';
      ...
      return '<div class="tool-run-badge">' +
        ...
        '</div>' +
        '<div class="tool-params">Params: ' + escapeHtml(params) + '</div>' +
        result +
      '</div>';
    }).join('');
    return '<div class="agent-logs-container">' +
      '<div class="agent-logs-header"><span>Execution Logs (' + toolCalls.length + ' operations)</span><span>open</span></div>' +
  ...
  function applyState(state) {
      ...
      renderClarificationPanel(state.awaitingClarification || null);
