...
const COMPANION_CLIENT_BUILD = 'operator-phone-mode-v28';
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #screen-new-chat.operator-mode .dispatch-chat-intro { display: flex; }
    #screen-new-chat.dispatch-mode .coder-workspace-picker,
    #screen-new-chat.operator-mode .coder-workspace-picker { display: none; }
...
<div class="app-root">
  ...
  <div class="drawer-overlay" id="app-drawer-overlay" role="dialog" aria-modal="true" aria-labelledby="app-drawer-title">
    <aside class="app-drawer">
      ...
      <nav class="drawer-nav" aria-label="Main">
        ...
        <button class="drawer-nav-btn" data-drawer-destination="operator" type="button">
          <span>Operator</span><span class="drawer-nav-sub">Desktop &amp; Browser</span>
        </button>
  ...
  <div id="screen-home" class="screen">
    ...
    <div class="home-body" id="home-body">
      <!-- Dispatch / Coder / Operator mode toggle -->
      <div class="mode-toggle-row" id="mode-toggle-row">
        ...
        <button type="button" class="mode-toggle-btn" id="mode-toggle-operator" data-mode="operator">Operator</button>
      </div>
...
<script>
  ...
  const modeToggleOperatorEl     = document.getElementById('mode-toggle-operator');
  ...
  function normalizeCompanionMode(mode) {
    return mode === 'coder' || mode === 'operator' ? mode : 'orion';
  }
  ...
  function updateNewChatModeUI() {
    ...
    const isOperatorStart = companionMode === 'operator';
    if (newChatScreenEl) {
      ...
      newChatScreenEl.classList.toggle('coder-mode', !isDispatchStart && !isOperatorStart);
    }
    ...
    if (introTitle) introTitle.textContent = isOperatorStart ? 'Operate the screen directly.' : 'No task is too large.';
    ...
    if (newChatPromptEl) {
      newChatPromptEl.placeholder = isDispatchStart
        ? 'Ask Orion anything...'
        : (isOperatorStart ? 'Ask Operator to open, click, type, or navigate...' : 'What should we build?');
    }
  }
  ...
  function updateDrawerState() {
    ...
    document.querySelectorAll('[data-drawer-destination]').forEach(btn => {
      ...
      else if (destination === 'coder' || destination === 'operator') {
  ...
  function updateHomeScreen(state) {
    ...
    // Recent chats -- Dispatch, Coder, and Operator each only see their own conversations here.
    // Legacy conversations saved before the mode field existed default to Dispatch.
    homeRecentsTitleEl.textContent = companionMode === 'coder'
      ? 'Coder Chats'
      : (companionMode === 'operator' ? 'Operator Sessions' : 'Dispatch Discussions');
  ...
  function applyModeToggleUI() {
    ...
    modeToggleOperatorEl.classList.toggle('active', companionMode === 'operator');
    updateDrawerState();
  }
  applyModeToggleUI();
  ...
  modeToggleOperatorEl.addEventListener('click', () => setCompanionMode('operator'));
  ...
  function openConversation(id) {
    ...
    if (target) {
      companionMode = normalizeCompanionMode(target.mode || 'orion');
      localStorage.setItem('orionCompanionMode', companionMode);
      applyModeToggleUI();
      if (companionMode === 'orion') {
  ...
  function updateSpecialistTabVisibility(mode) {
    const isSpecialist = mode === 'coder' || mode === 'operator';
  ...
  function applyState(state) {
        ...
        ? (window.OrionTaskOrchestration
          && typeof window.OrionTaskOrchestration.describeSupervisedTaskPresentation === 'function'
            ? window.OrionTaskOrchestration.describeSupervisedTaskPresentation(supervisedTask, {
                ...
                roleLabel: supervisedTask.target && supervisedTask.target.mode === 'operator' ? 'Operator' : 'Coder'
              })
      ...
      if (showBackgroundCoder) {
        ...
        dispatchRunningBanner.classList.toggle('operator-control', supervisedRole === 'operator');
        ...
        dispatchRunningText.textContent = (supervisedRole === 'operator' ? 'Screen control · ' : '')
          + supervisedPresentation.label + ': ' + (supervisedTask.title || (supervisedRole === 'operator' ? 'Operator task' : 'Coder task'));
      } else {
