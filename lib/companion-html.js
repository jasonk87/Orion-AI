...
const COMPANION_CLIENT_BUILD = 'optimistic-phone-send-v25';
...
<script>
  ...
  function renderMessageImages(images, conversationId) {
  ...
  async function loadState(options = {}) {
    ...
    if (!options.force && ((sseActive && sseStateReceived && Date.now() - lastSseActivityAt < 25000) || stateFetchController)) return;
  ...
  async function startEventStream(options = {}) {
    if (!deviceSession) {
      scheduleEventStreamReconnect(eventStreamGeneration, 800);
      return;
    }
  ...
  function recoverForegroundConnection() {
    ...
    foregroundRecoveryTimer = setTimeout(() => {
      ...
      setTimeout(() => {
        if (!sseStateReceived || lastSseMessageAt < recoveryStartedAt) loadState({ force: true });
      }, 650);
    }, 50);
  }
