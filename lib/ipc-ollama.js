    ...
    try {
      ...
      await Promise.all(Array.from({ length: Math.min(4, installed.length) }, async () => {
        while (index < installed.length && !controller.signal.aborted) {
          ...
          let contextLength = capabilities ? cached.contextLength : null;
          if (!capabilities) {
            try {
              ...
              contextLength = Number(info.model_info?.[`${architecture}.context_length`]) || null;
              if (capabilities) details.set(model.name, { digest: model.digest, capabilities, contextLength });
            } catch (_) { /* A failed capability lookup must not hide an installed model. */ }
          }
          models.push({ name: model.name, value: `ollama:${model.name}`, capabilities, contextLength });
        }
      }));
