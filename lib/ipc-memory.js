...
function registerHandlers(ipcMain) {
  ...
  ipcMain.handle('orion:append-global-preference', async (event, { text, config, source, mode } = {}) => {
    if (!text) return { success: false, error: 'text is required' };
    try {
      const result = await mm.appendGlobalPreference(text, config, source, mode);
      return { success: true, ...result };
    } catch (e) {
      return { success: false, error: e.message };
    }
  ...
  ipcMain.handle('orion:append-project-preference', async (event, { workspacePath, text, config, mode } = {}) => {
    if (!workspacePath) return { success: false, error: 'workspacePath is required' };
    if (!text) return { success: false, error: 'text is required' };
    try {
      const result = await mm.appendProjectPreference(workspacePath, text, config, mode);
      return { success: true, ...result };
    } catch (e) {
      return { success: false, error: e.message };
    }
