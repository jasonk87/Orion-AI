...
function registerHandlers(ipcMain) {
  ...
  ipcMain.handle('orion:append-global-fact', async (event, { text, category, config } = {}) => {
    if (!text) return { success: false, error: 'text is required' };
    try {
      const result = await mm.appendGlobalFact(text, category, config);
      return { success: true, ...result };
    } catch (e) {
      return { success: false, error: e.message };
    }
  ...
  ipcMain.handle('orion:append-global-preference', async (event, { text, config, source } = {}) => {
    if (!text) return { success: false, error: 'text is required' };
    try {
      const result = await mm.appendGlobalPreference(text, config, source);
      return { success: true, ...result };
    } catch (e) {
      return { success: false, error: e.message };
    }
  ...
  ipcMain.handle('orion:rank-memory-facts', async (event, { query, config, limit } = {}) => {
    try {
      const results = await mm.rankGlobalFactsByQuery(query, config, limit || 10);
  ...
  ipcMain.handle('orion:append-project-fact', async (event, { workspacePath, text, category, config } = {}) => {
    if (!workspacePath) return { success: false, error: 'workspacePath is required' };
    if (!text) return { success: false, error: 'text is required' };
    try {
      const result = await mm.appendProjectFact(workspacePath, text, category, config);
      return { success: true, ...result };
    } catch (e) {
      return { success: false, error: e.message };
    }
  ...
  ipcMain.handle('orion:append-project-memory', async (event, { workspacePath, fact } = {}) => {
    ...
    try {
      const result = await mm.appendProjectFact(workspacePath, fact.text, fact.category);
      return { success: true, ...result };
    } catch (e) {
      return { success: false, error: e.message };
    }
  ...
  ipcMain.handle('orion:append-project-preference', async (event, { workspacePath, text, config } = {}) => {
    if (!workspacePath) return { success: false, error: 'workspacePath is required' };
    if (!text) return { success: false, error: 'text is required' };
    try {
      const result = await mm.appendProjectPreference(workspacePath, text, config);
      return { success: true, ...result };
    } catch (e) {
      return { success: false, error: e.message };
    }
