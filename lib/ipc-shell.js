...
} = require('./windows-window-capture');
...
function closeBrowserWorkerInstance(win, expectedUrlContains = '') {
  if (!win || win.isDestroyed()) {
    return { success: true, closed: false, alreadyClosed: true };
  }

  const url = String(win.webContents && typeof win.webContents.getURL === 'function'
    ? win.webContents.getURL()
    : '');
  const title = String(typeof win.getTitle === 'function' ? win.getTitle() : '');
  const expected = String(expectedUrlContains || '').trim().toLowerCase();
  if (expected && !url.toLowerCase().includes(expected) && !title.toLowerCase().includes(expected)) {
    return {
      success: false,
      closed: false,
      error: `The managed browser is showing "${title || url || 'an unknown page'}", not the expected page "${expectedUrlContains}".`,
      url,
      title
    };
  }

  // This is Orion's isolated browser worker, not the user's browser. Destroy it directly so a
  // page-level beforeunload handler cannot turn a verified close into an apparent success.
  win.destroy();
  return { success: true, closed: true, url, title };
}

function browserClose(expectedUrlContains = '') {
  const result = closeBrowserWorkerInstance(browserWorker, expectedUrlContains);
  if (result.success && result.closed) browserWorker = null;
  return result;
}
...
async function captureElectronWindowByHint(windowHint, thumbnailSize, desktopCapturerImpl) {
  const hint = String(windowHint || '').trim().toLowerCase();
  if (!hint) throw new Error('A named application window is required for window capture.');
  const sources = await desktopCapturerImpl.getSources({ types: ['window'], thumbnailSize });
  const candidates = (Array.isArray(sources) ? sources : [])
    .map(source => ({ source, name: String(source && source.name || '').trim() }))
    .filter(item => item.name && item.name.toLowerCase().includes(hint))
    .map(item => ({ ...item, score: item.name.toLowerCase() === hint ? 0 : 1 }));
  if (!candidates.length) throw new Error(`No capturable application window matched "${windowHint}".`);
  const bestScore = Math.min(...candidates.map(item => item.score));
  const best = candidates.filter(item => item.score === bestScore);
  if (best.length !== 1) throw new Error(`More than one capturable application window matched "${windowHint}".`);
  const image = best[0].source && best[0].source.thumbnail;
  if (!image || image.isEmpty()) throw new Error(`Application window "${best[0].name}" returned an empty image.`);
  return {
    png: image.toPNG(),
    size: image.getSize(),
    windowTitle: best[0].name,
    processName: ''
  };
}
...
async function captureDesktopScreenshot(workspacePath, destination, prefix = 'preview', {
  ...
  // A first empty framebuffer read is not proof the desktop is uncapturable: Windows' Desktop
  // Duplication API (what desktopCapturer uses) can transiently hand back a blank frame during a
  // secure-desktop transition (a UAC prompt or the lock screen briefly owns the swap chain), right
  // after a display-mode/resolution change, or while a fullscreen-exclusive GPU app briefly owns
  // the compositor. All of these typically clear within a few hundred milliseconds. A bare desktop
  // request has no window-hint fallback to fall back to, so without this it fails on the very
  // first transient blank frame with no recovery attempt at all. Defaults are production values;
  // tests override retryDelayMs to 0 so this stays fast and deterministic.
  captureRetries = 2,
  retryDelayMs = 300
} = {}) {
  const { desktopCapturer, screen } = require('electron');

  // During a task-scoped Operator session Orion is already minimized. Showing it after each
  // capture would reclaim focus between actions; that session alone owns restoration.
  const shouldHide = hideOrion && shared.mainWindow && !shared.mainWindow.isDestroyed()
    && !operatorControlSession.isControlSessionActive();
  if (shouldHide) {
  ...
  try {
    ...
    // Multi-monitor targeting: an explicit displayId picks a non-primary monitor, threaded from
    // the capture_screen result the model was shown (see agent.js's lastDesktopSnapshot). An
    // An omitted id deliberately means primary. An explicit id is evidence-bound: if that monitor
    // disappeared, silently switching to primary would capture a different, uninspected surface.
    const allDisplays = typeof screen.getAllDisplays === 'function' ? screen.getAllDisplays() : [primary];
    const hasExplicitDisplay = displayId !== undefined && displayId !== null && displayId !== '';
    const target = hasExplicitDisplay
      ? allDisplays.find(d => String(d.id) === String(displayId))
      : primary;
    if (!target) {
      const error = new Error(`Display ${displayId} is no longer connected. Capture and inspect a currently available display before acting.`);
    ...
    // A blank first read gets a few short, cheap retries before anything else is tried or given
    // up on — see the captureRetries/retryDelayMs comment above for why. Each attempt re-queries
    // desktopCapturer fresh rather than reusing the first (empty) result.
    const totalAttempts = Math.max(1, 1 + (Number.isFinite(captureRetries) ? captureRetries : 0));
    let sources = [];
    let image = null;
    for (let attempt = 0; attempt < totalAttempts; attempt++) {
      sources = await desktopCapturer.getSources({ types: ['screen'], thumbnailSize });
      const targetSource = sources.find(s => String(s.display_id) === String(target.id)) || sources[0] || null;
      image = targetSource && targetSource.thumbnail;
    ...
    if (!image || image.isEmpty()) {
      // Electron's DXGI/Desktop Duplication path can remain empty for the entire session even
      // while an interactive Windows desktop is plainly visible (the exact phone/Operator failure
      // this fallback addresses). A full-display request must remain full-display evidence, so use
      // a second OS capture backend before considering the narrower named-window fallback.
      const captureDisplay = typeof displayCapture === 'function'
        ? displayCapture
        : captureDisplayByBounds;
      let displayShot = null;
      let displayError = null;
      try {
        displayShot = await captureDisplay(target.bounds || {
          x: 0,
          y: 0,
          width: target.size && target.size.width || thumbnailSize.width,
          height: target.size && target.size.height || thumbnailSize.height
        });
      } catch (error) {
        displayError = error;
      }
      if (displayShot && displayShot.png) {
        const fallback = path.join('screenshots', `${prefix}-${Date.now()}.png`);
        const written = writeScreenshotBuffer({
          workspacePath,
          conversationId,
          destination,
          fallback,
        ...
        return {
          ...
          png: displayShot.png,
          bitmap: null,
          size: displayShot.size,
          artifactPath: written.artifactPath,
          artifactRelativePath: written.artifactRelativePath,
          displayId: target.id,
          availableDisplays: allDisplays.map(d => ({
            id: d.id,
          ...
          captureMode: 'display',
          captureBackend: displayShot.backend === 'window_composite'
            ? 'windows_window_composite'
            : 'windows_native'
        };
      }
      if (!windowHint) {
        const captureForeground = typeof foregroundCapture === 'function'
          ? foregroundCapture
          : captureForegroundWindow;
        try {
          const foregroundShot = await captureForeground();
          const fallback = path.join('screenshots', `${prefix}-${Date.now()}.png`);
          const written = writeScreenshotBuffer({
            workspacePath,
            conversationId,
            destination,
            fallback,
            png: foregroundShot.png
          });
          return {
            rel: written.rel,
            png: foregroundShot.png,
            bitmap: null,
            size: foregroundShot.size,
            artifactPath: written.artifactPath,
            artifactRelativePath: written.artifactRelativePath,
            displayId: target.id,
            availableDisplays: allDisplays.map(d => ({
            ...
            captureBackend: 'windows_foreground',
            windowTitle: foregroundShot.windowTitle || '',
            processName: foregroundShot.processName || ''
          };
        } catch (foregroundError) {
          const electronError = sources.length
            ? 'Captured screen image was empty.'
            : 'No screen sources available for capture.';
          const nativeDisplayDetail = displayError
            ? ` Native desktop fallback also failed: ${displayError.message || displayError}.`
            : '';
          throw new Error(`${electronError}${nativeDisplayDetail} Foreground-window fallback also failed: ${foregroundError.message || foregroundError}`);
        }
      }
      const captureWindow = typeof windowCapture === 'function'
        ? windowCapture
        : async hint => {
          // Electron's per-window thumbnail is the same capture path preview_app successfully uses
          // for SDL/GPU applications. Prefer it before PrintWindow, which often cannot render a
          // Direct3D-backed surface even though the native window is alive and visible.
          try {
            return await captureElectronWindowByHint(hint, thumbnailSize, desktopCapturer);
          } catch (electronWindowError) {
            try {
              return await captureWindowByHint(hint);
            } catch (nativeWindowError) {
              throw new Error(`${electronWindowError.message || electronWindowError} Native fallback: ${nativeWindowError.message || nativeWindowError}`);
            }
          }
        };
      let windowShot;
      try {
        windowShot = await captureWindow(windowHint);
      } catch (windowError) {
        const nativeDetail = displayError
          ? ` Native desktop fallback: ${displayError.message || displayError}.`
          : '';
        throw new Error(`Captured screen image was empty, and the named application window fallback also failed: ${windowError.message || windowError}.${nativeDetail}`);
      }
      const fallback = path.join('screenshots', `${prefix}-${Date.now()}.png`);
      const written = writeScreenshotBuffer({
        workspacePath,
      ...
      return {
        ...
        png: windowShot.png,
        bitmap: null,
        size: windowShot.size,
        artifactPath: written.artifactPath,
        artifactRelativePath: written.artifactRelativePath,
        // A cropped application window is valid evidence for reading/attaching that window, but
        // it is not a monitor coordinate map. Keep the display identity only as metadata and let
        // the renderer-side computer_action gate reject this capture mode for pixel input.
        displayId: target.id,
        availableDisplays: allDisplays.map(d => ({
          id: d.id,
          primary: d.id === primary.id,
        ...
        captureMode: 'application_window',
        windowTitle: windowShot.windowTitle || '',
        processName: windowShot.processName || ''
      };
    }
    const fallback = path.join('screenshots', `${prefix}-${Date.now()}.png`);
    const png = image.toPNG();
...
async function previewWorkspaceApp(workspacePath, { command, warmupMs, destination, processId, timeoutMs, conversationId } = {}, getWorkspaceEntrypoint, readAppConfigFn) {
  ...
  try {
    ...
    if (newWins.length > 0) {
      ...
      shot = {
        ...
        artifactRelativePath: written.artifactRelativePath,
        captureMode: 'application_window',
        windowTitle: String(appSource.name || '')
      };
    } else {
  ...
  return {
    ...
    windowTitle: shot ? String(shot.windowTitle || '') : '',
    processName: shot ? String(shot.processName || '') : '',
    displayId: shot && shot.displayId != null ? String(shot.displayId) : '',
...
async function captureScreenForAgent(workspacePath, { destination, delayMs, conversationId, displayId, windowHint } = {}) {
  ...
  try {
    const shot = await captureDesktopScreenshot(workspacePath, destination, 'capture', {
      ...
      conversationId,
      displayId,
      // preview_app already identified the exact native window it launched. Preserve that trusted
      // observation so a later capture can use PrintWindow when Windows temporarily returns an
      // empty display thumbnail (locked/dimmed display, GPU composition, or remote-session gap).
      // This fallback is image evidence only; captureMode prevents it from authorizing pixel input.
      windowHint
    });
    ...
    return {
      ...
      availableDisplays: shot.availableDisplays || [],
      captureMode: shot.captureMode || 'display',
      windowTitle: String(shot.windowTitle || ''),
      processName: String(shot.processName || ''),
      summary: shot.captureMode === 'application_window'
        ? `Captured the foreground application window${shot.windowTitle ? ` "${shot.windowTitle}"` : ''} to ${shot.rel} (${shot.size.width}x${shot.size.height})${wait ? ` after waiting ${wait}ms` : ''}. The full desktop framebuffer was unavailable, so this image is window evidence rather than monitor-coordinate evidence.`
        : `Captured the current screen to ${shot.rel} (${shot.size.width}x${shot.size.height})${wait ? ` after waiting ${wait}ms` : ''}.`
    };
  } catch (err) {
...
function registerHandlers(ipcMain, { getWorkspaceEntrypoint, readAppConfig: readAppConfigFn } = {}) {
  ...
  ipcMain.handle('kill-command', (event, processId) => {
    ...
    if (shared.activeProcesses[resolvedId]) {
      try {
        const rawPid = shared.commandSessions[resolvedId] && shared.commandSessions[resolvedId].pid
          ? String(shared.commandSessions[resolvedId].pid)
          : (shared.activeProcesses[resolvedId].pid ? String(shared.activeProcesses[resolvedId].pid) : '');
        ...
        return { success: true, pid: rawPid || null };
      } catch (e) {
  ...
  ipcMain.handle('browser-close', async (event, { expectedUrlContains } = {}) => {
    try { return browserClose(expectedUrlContains); } catch (e) { return { success: false, error: e.message }; }
  });
  ...
  ipcMain.handle('capture-screen', async (event, { workspacePath, destination, delayMs, conversationId, displayId, windowHint }) => {
    try { return await captureScreenForAgent(workspacePath, { destination, delayMs, conversationId, displayId, windowHint }); } catch (e) { return { success: false, error: e.message }; }
  });
...
module.exports = {
  ...
  captureElectronWindowByHint,
  previewWorkspaceApp,
  pickBestClickCandidate,
  closeBrowserWorkerInstance,
  isProcessAlive,
