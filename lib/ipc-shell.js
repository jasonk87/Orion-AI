...
const { captureWindowByHint } = require('./windows-window-capture');
...
async function captureDesktopScreenshot(workspacePath, destination, prefix = 'preview', {
  ...
  try {
    ...
    const image = targetSource && targetSource.thumbnail;
    if (!image || image.isEmpty()) {
      ...
      const captureWindow = typeof windowCapture === 'function' ? windowCapture : captureWindowByHint;
      let windowShot;
      try {
        windowShot = await captureWindow(windowHint);
      } catch (windowError) {
        throw new Error(`Captured screen image was empty, and the named application window fallback also failed: ${windowError.message || windowError}`);
      }
...
async function captureScreenForAgent(workspacePath, { destination, delayMs, conversationId, displayId } = {}) {
  ...
  try {
    const shot = await captureDesktopScreenshot(workspacePath, destination, 'capture', { hideOrion: true, conversationId, displayId });
...
function registerHandlers(ipcMain, { getWorkspaceEntrypoint, readAppConfig: readAppConfigFn } = {}) {
  ...
  ipcMain.handle('capture-screen', async (event, { workspacePath, destination, delayMs, conversationId, displayId }) => {
    try { return await captureScreenForAgent(workspacePath, { destination, delayMs, conversationId, displayId }); } catch (e) { return { success: false, error: e.message }; }
  });
