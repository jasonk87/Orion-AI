...
async function captureDesktopScreenshot(workspacePath, destination, prefix = 'preview', { hideOrion = false, conversationId = '', displayId = '' } = {}) {
  ...
  try {
    ...
    // unset or stale id (monitor unplugged, no id supplied) falls back to the primary display
    // rather than erroring, matching the fail-safe default used everywhere else in this file.
    const allDisplays = typeof screen.getAllDisplays === 'function' ? screen.getAllDisplays() : [primary];
    const target = (displayId !== undefined && displayId !== null && displayId !== '')
      ? (allDisplays.find(d => String(d.id) === String(displayId)) || primary)
      : primary;
