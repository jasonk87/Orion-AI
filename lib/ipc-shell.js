...
const { safeRelativeAssetPath, downloadFileToWorkspace } = require('./ipc-file-tools');
...
function pruneCommandSessions() {
  const completedIds = Object.keys(shared.commandSessions)
    ...
    .sort((a, b) => (shared.commandSessions[a].finishedAt || shared.commandSessions[a].startedAt) - (shared.commandSessions[b].finishedAt || shared.commandSessions[b].startedAt));
...
async function previewWorkspaceApp(workspacePath, { command, warmupMs, destination, processId, timeoutMs } = {}, getWorkspaceEntrypoint, readAppConfigFn) {
...
function registerHandlers(ipcMain, { getWorkspaceEntrypoint, readAppConfig: readAppConfigFn } = {}) {
  ...
  ipcMain.handle('take-screenshot', async (event, { workspacePath, destination }) => {
    try { return await takeBrowserScreenshot(workspacePath, destination); } catch (e) { return { success: false, error: e.message }; }
  });

  ipcMain.handle('preview-workspace-app', async (event, { workspacePath, command, warmupMs, destination, processId, timeoutMs }) => {
    try {
      return await previewWorkspaceApp(workspacePath, { command, warmupMs, destination, processId, timeoutMs }, getWorkspaceEntrypoint, readAppConfigFn);
    } catch (e) {
      return { success: false, error: e.message };
    }
  });
  ...
  ipcMain.handle('capture-screen', async (event, { workspacePath, destination, delayMs }) => {
    try { return await captureScreenForAgent(workspacePath, { destination, delayMs }); } catch (e) { return { success: false, error: e.message }; }
  });
}
