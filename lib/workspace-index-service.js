  ...
  persist() {
    ...
    try {
      ...
      for (const [relPath, record] of this.records.entries()) {
        const available = Math.max(0, MAX_PERSISTED_SEMANTIC_CHUNKS - persistedSemanticChunks);
        const semanticChunks = Array.isArray(record.semanticChunks) && available > 0
          ? record.semanticChunks.slice(0, available)
          : undefined;
        persistedSemanticChunks += semanticChunks ? semanticChunks.length : 0;
        files[relPath] = { ...record, semanticChunks, sourceSnapshot: undefined, dirty: undefined };
      }
      const payload = {
        ...
        fileKnowledge: Object.fromEntries(this.fileKnowledge.entries()),
        files
      };
      atomicWriteJsonSync(filePath, payload, { trailingNewline: true });
    } catch (_) {}
  }
