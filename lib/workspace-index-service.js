...
const MAX_TRACKED_FILE_KNOWLEDGE = 200;
const MAX_DIGEST_CHARS = 400;
const DEFAULT_EMBEDDING_CONCURRENCY = 4;
const MAX_PERSISTED_SEMANTIC_CHUNKS = 2500;
const DEFAULT_PERSISTED_CACHE_MAX_BYTES = 16 * 1024 * 1024;
...
function createEmptyTelemetry() {
  return {
    ...
    persistedCacheBytes: 0,
    persistedFilesOmitted: 0
  };
}
...
class WorkspaceIndexService {
  constructor(workspacePath, options = {}) {
    ...
    this.options = {
      ...
      deferInitialReconcile: options.deferInitialReconcile === true,
      embeddingConcurrency: Math.max(1, Math.min(Number(options.embeddingConcurrency) || DEFAULT_EMBEDDING_CONCURRENCY, 8)),
      persistedCacheMaxBytes: Math.max(1024, Number(options.persistedCacheMaxBytes) || DEFAULT_PERSISTED_CACHE_MAX_BYTES)
    };
    ...
    this.initialReconcilePending = false;
    this.initialReconcileHandle = null;
    this.loadPersistedCache();
    this.loadLegacyFileKnowledge();
    if (this.options.deferInitialReconcile) {
      this.initialReconcilePending = true;
      this.initialReconcileHandle = setImmediate(() => {
        this.initialReconcileHandle = null;
        if (!this.closed) this.reconcile();
      });
    } else {
      this.reconcile();
    }
  ...
  loadPersistedCache() {
    ...
    try {
      ...
      for (const [relPath, knowledge] of Object.entries(parsed.fileKnowledge || {})) {
        if (knowledge && knowledge.hash) this.fileKnowledge.set(normalizeSlash(relPath), knowledge);
      }
      const now = Date.now();
  ...
  persist() {
    ...
    try {
      ...
      const maxBytes = this.options.persistedCacheMaxBytes;
      const jsonBytes = value => Buffer.byteLength(JSON.stringify(value), 'utf8');
      let persistedBytes = jsonBytes(payload) + 1;

      const appendArrayItem = (target, value) => {
        const contribution = jsonBytes(value) + (target.length > 0 ? 1 : 0);
        if (persistedBytes + contribution > maxBytes) return false;
        target.push(value);
        persistedBytes += contribution;
        return true;
      };
      const appendObjectEntry = (target, key, value, hasEntries) => {
        const contribution = jsonBytes(String(key)) + 1 + jsonBytes(value)
          + (hasEntries ? 1 : 0);
        if (persistedBytes + contribution > maxBytes) return false;
        target[key] = value;
        persistedBytes += contribution;
      ...
      const contextPackets = [...this.contextPackets.values()]
        .sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
      for (const packet of contextPackets) appendArrayItem(payload.contextPackets, packet);

      const knowledgeEntries = [...this.fileKnowledge.entries()]
        .sort((a, b) => Number(b[1]?.lastReadAt || 0) - Number(a[1]?.lastReadAt || 0)
          || a[0].localeCompare(b[0]));
      let persistedKnowledge = 0;
      for (const [relPath, knowledge] of knowledgeEntries) {
        if (appendObjectEntry(payload.fileKnowledge, relPath, knowledge, persistedKnowledge > 0)) {
          persistedKnowledge += 1;
        }
      }

      let persistedSemanticChunks = 0;
      const records = [...this.records.entries()].sort((a, b) => {
        const aSemantic = Array.isArray(a[1]?.semanticChunks) && a[1].semanticChunks.length > 0 ? 1 : 0;
        const bSemantic = Array.isArray(b[1]?.semanticChunks) && b[1].semanticChunks.length > 0 ? 1 : 0;
      ...
      for (const [relPath, record] of records) {
        ...
        const sourceChunks = Array.isArray(persistedRecord.semanticChunks)
          ? persistedRecord.semanticChunks : [];
        delete persistedRecord.semanticChunks;
        delete persistedRecord.sourceSnapshot;
        delete persistedRecord.dirty;

        const keyContribution = jsonBytes(String(relPath)) + 1
          + (persistedFiles > 0 ? 1 : 0);
        const baseContribution = keyContribution + jsonBytes(persistedRecord);
        if (persistedBytes + baseContribution > maxBytes) continue;

        const availableChunks = Math.max(0, MAX_PERSISTED_SEMANTIC_CHUNKS - persistedSemanticChunks);
        const selectedChunks = [];
        if (availableChunks > 0 && sourceChunks.length > 0) {
          const semanticPropertyOverhead = (Object.keys(persistedRecord).length > 0 ? 1 : 0)
            + jsonBytes('semanticChunks') + 1 + 2;
          let recordContribution = baseContribution + semanticPropertyOverhead;
          if (persistedBytes + recordContribution <= maxBytes) {
            for (const chunk of sourceChunks.slice(0, availableChunks)) {
              const chunkContribution = jsonBytes(chunk) + (selectedChunks.length > 0 ? 1 : 0);
              if (persistedBytes + recordContribution + chunkContribution > maxBytes) break;
              selectedChunks.push(chunk);
              recordContribution += chunkContribution;
            }
          }
        }
        if (selectedChunks.length > 0) persistedRecord.semanticChunks = selectedChunks;
        if (!appendObjectEntry(payload.files, relPath, persistedRecord, persistedFiles > 0)) continue;
        persistedFiles += 1;
        persistedSemanticChunks += selectedChunks.length;
      }

      this.telemetry.persistedFilesOmitted = Math.max(0, this.records.size - persistedFiles);
      this.telemetry.persistedCacheBytes = persistedBytes;
      atomicWriteJsonSync(filePath, payload, { trailingNewline: true, space: 0 });
    } catch (_) {}
  }

  loadLegacyFileKnowledge() {
    const legacyPath = path.join(this.workspacePath, '.orion', 'file-knowledge.json');
    if (!fs.existsSync(legacyPath)) return;
    try {
      const legacy = JSON.parse(fs.readFileSync(legacyPath, 'utf8'));
      for (const [relPath, knowledge] of Object.entries(legacy.files || {})) {
        const normalized = normalizeSlash(relPath);
        if (!this.fileKnowledge.has(normalized) && knowledge && knowledge.hash) {
          this.fileKnowledge.set(normalized, knowledge);
        }
      }
      this.pruneFileKnowledge();
    } catch (_) {}
  }

  pruneFileKnowledge() {
    const entries = [...this.fileKnowledge.entries()]
      .sort((a, b) => Number(b[1].lastReadAt || 0) - Number(a[1].lastReadAt || 0));
    for (const [relPath] of entries.slice(MAX_TRACKED_FILE_KNOWLEDGE)) this.fileKnowledge.delete(relPath);
  }

  recordFileRead(relativePath) {
    const relPath = normalizeSlash(relativePath).replace(/^\.\//, '');
    if (!relPath) throw new Error('Missing relative path');
    resolveWorkspacePath(this.workspacePath, relPath);
    const { record } = this.readSource(relPath);
    if (!record) throw new Error(`Not an indexable file: ${relativePath}`);
    const existing = this.fileKnowledge.get(relPath);
    const keepDigest = existing && existing.hash === record.hash ? existing : null;
    this.fileKnowledge.set(relPath, {
      hash: record.hash,
    ...
    this.pruneFileKnowledge();
    this.persist();
    return { recorded: relPath, hash: record.hash, hadCurrentDigest: !!(keepDigest && keepDigest.digest) };
  }

  saveFileDigest(relativePath, digest) {
    const relPath = normalizeSlash(relativePath).replace(/^\.\//, '');
    ...
    this.pruneFileKnowledge();
    this.persist();
    return { saved: relPath, hash: record.hash };
  }

  buildKnowledgeBrief({ maxDigests = 25 } = {}) {
    const knownCurrent = [];
    const seenCurrent = [];
    const changed = [];
    const missing = [];
    for (const [relPath, knowledge] of this.fileKnowledge.entries()) {
      let stat;
      try {
        stat = fs.statSync(resolveWorkspacePath(this.workspacePath, relPath));
      } catch (_) {
      ...
      if (!current) {
        try { current = this.readSource(relPath).record.hash === knowledge.hash; } catch (_) { current = false; }
      }
    ...
    return {
      ...
      totalTracked: this.fileKnowledge.size
    };
  }
  ...
  close() {
    this.closed = true;
    if (this.initialReconcileHandle) clearImmediate(this.initialReconcileHandle);
    this.initialReconcileHandle = null;
    if (this.debounceTimer) clearTimeout(this.debounceTimer);
    this.debounceTimer = null;
    if (this.watcher) {
  ...
  ensureInitialReconciled() {
    if (!this.initialReconcilePending || this.closed) return;
    if (this.initialReconcileHandle) clearImmediate(this.initialReconcileHandle);
    this.initialReconcileHandle = null;
    this.reconcile();
  }
  ...
  async getSemanticChunks(config = {}, generateEmbedding) {
    ...
    for (const record of this.listRecords()) {
      ...
      pendingRecords.push({ record, rawChunks, embedded: new Array(rawChunks.length) });
    }
    const jobs = pendingRecords.flatMap(item => item.rawChunks.map((chunk, chunkIndex) => ({ item, chunk, chunkIndex })));
    for (let index = 0; index < jobs.length; index += this.options.embeddingConcurrency) {
      await Promise.all(jobs.slice(index, index + this.options.embeddingConcurrency).map(async ({ item, chunk, chunkIndex }) => {
        const vector = await generateEmbedding(chunk.text, config);
        item.embedded[chunkIndex] = {
          ...chunk,
          vector,
          sourceHash: item.record.hash,
    ...
    for (const item of pendingRecords) {
      item.record.semanticChunks = item.embedded.filter(Boolean);
      chunks.push(...item.record.semanticChunks.map(chunk => ({ ...chunk, file: item.record.path })));
      changed = true;
    }
...
function getWorkspaceIndexService(workspacePath, options = {}) {
  ...
  if (!service || service.closed || options.fresh) {
    ...
    service = new WorkspaceIndexService(key, {
      ...options,
      deferInitialReconcile: options.deferInitialReconcile !== false
    });
