...
async function execute(operation, payload = {}) {
  ...
  switch (operation) {
    ...
    case 'recordFileRead':
      return {
        success: true,
        ...recordFileRead(workspacePath, payload.relativePath)
      };
    case 'saveFileDigest':
      return {
        success: true,
        ...saveFileDigest(workspacePath, payload.relativePath, payload.digest)
      };
    case 'getKnowledgeBrief':
      return {
        success: true,
        ...buildKnowledgeBrief(workspacePath, { maxDigests: payload.maxDigests })
      };
    case 'semanticSearch':
      return {
        success: true,
