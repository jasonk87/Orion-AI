...
function buildCaptureWindowScript(windowHint) {
  ...
  return `$ErrorActionPreference = 'Stop'\n` +
    ...
    `$matches = @(Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 -and ($_.ProcessName.IndexOf($hint, [StringComparison]::OrdinalIgnoreCase) -ge 0 -or $_.MainWindowTitle.IndexOf($hint, [StringComparison]::OrdinalIgnoreCase) -ge 0) })\n` +
    `if ($matches.Count -eq 0) { [pscustomobject]@{ success=$false; reasonCode='window_not_found'; error="No visible application window matched '$hint'." } | ConvertTo-Json -Compress; exit 0 }\n` +
    `$chosen = @($matches | Sort-Object @{Expression={ if ($_.ProcessName -ieq $hint -or $_.MainWindowTitle -ieq $hint) { 0 } else { 1 } }}, StartTime | Select-Object -First 1)[0]\n` +
    ...
    `  [pscustomobject]@{ success=$true; width=$width; height=$height; windowTitle=$chosen.MainWindowTitle; processName=$chosen.ProcessName; pngBase64=$png } | ConvertTo-Json -Compress\n` +
