...
async function notifyPhoneDevice(deviceId, title, body) {
  if (!webPush || !_vapidKeys) return { success: false, reason: 'web-push not available' };
  const config = readAppConfig();
...
async function notifyAllPhoneDevices(title, body) {
  if (!webPush || !_vapidKeys) return;
  const config = readAppConfig();
  const active = (config.phoneCompanionDevices || []).filter(d => !d.revoked && d.pushSubscription);
  await Promise.allSettled(active.map(d => notifyPhoneDevice(d.id, title, body)));
}
...
function companionServiceWorker() {
  return `const CACHE = 'orion-phone-companion-v15';
  ...
  shared.companionServer = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/vapid-public-key') {
        const publicKey = (_vapidKeys && _vapidKeys.publicKey) || (readAppConfig().vapidPublicKey) || null;
        sendJson(res, 200, { success: true, publicKey });
        return;
      }
...
function registerHandlers(ipcMain) {
  ...
  ipcMain.handle('revoke-all-phone-companion-devices', async () => {
    const config = readAppConfig();
    const devices = getCompanionDevices(config).map(d => ({ ...d, revoked: true }));
    config.phoneCompanionDevices = devices;
    writeAppConfig(config);
    return { success: true };
  });
  ...
  ipcMain.handle('notify-phone', async (event, { title, body } = {}) => {
    return await notifyAllPhoneDevices(String(title || 'Orion AI'), String(body || ''));
  });
