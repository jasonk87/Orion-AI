...
function resolveCompanionActionConversation(device, requestedConversationId) {
...
function companionServiceWorker() {
  return `const CACHE = 'orion-phone-companion-v24';
  ...
  shared.companionServer = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/api/push-subscribe') {
        ...
        await updateAppConfig(config => {
          config.phoneCompanionDevices = getCompanionDevices(config).map(candidate => (
            candidate.id === device.id
              ? { ...candidate, pushSubscription: body.subscription }
              : candidate
          ));
          return config;
        });
      ...
      if (req.method === 'GET' && url.pathname === '/api/chat-image') {
        ...
        const scope = resolveCompanionActionConversation(device, requestedConversationId);
        if (!scope.success) {
