...
function companionServiceWorker() {
  return `const CACHE = 'orion-phone-companion-v26';
const SHELL = ['/icon.svg', '/icon-192.png', '/icon-512.png', '/marked.min.js', '/task-orchestration.js'];
  ...
  shared.companionServer = http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/') {
        ...
        res.end(companionHtml(os.hostname()));
        return;
      }
      ...
      if (req.method === 'POST' && url.pathname === '/api/pair') {
        ...
        const device = await createCompanionDeviceSession(deviceName, userAgent);
      ...
      if (req.method === 'POST' && url.pathname === '/api/prompt') {
        ...
        const result = await callRendererFunction('submitPhoneCompanionPrompt', {
          ...
          mode: body.mode === 'coder' || body.mode === 'operator' ? body.mode : 'orion',
      ...
      if (req.method === 'GET' && url.pathname === '/api/chat-image') {
        ...
        // attached by a delegated Operator/Coder task keeps that specialist conversation's id as
        // its sourceConversationId even after the result is relayed into the Dispatch transcript
        // the phone has selected (see attach_image in agent.js and companion-html.js's
        // data-chat-image-conversation), so a legitimate request routinely names a conversation
        // other than the one currently selected. Requiring them to match rejected every such
        // relayed image with a permanent 409, which the phone reported as "Image unavailable"
        // after exhausting its retries - not a race, a guaranteed rejection. The device is already
        // authenticated above, and readChatImageForPhone independently verifies both that this
        // exact conversation exists and that this exact path is actually attached to one of its
        // messages, so the real authorization boundary is preserved without the stale-view check.
  ...
  return new Promise((resolve, reject) => {
    ...
    const pairingAnnouncementTimer = setTimeout(async () => {
      if (shared.mainWindow && !shared.mainWindow.isDestroyed()) {
        try {
          const currentConfig = readAppConfig();
          const secureOrigin = currentConfig.phoneCompanionHttpsOrigin || '';
...
async function getPhoneCompanionPairingPayload() {
  ...
  const secureOrigin = latestConfig.phoneCompanionHttpsOrigin || '';
  const secureOriginReachable = await verifyCompanionHttpsOrigin(secureOrigin);
  return {
    success: true,
    enabled: true,
