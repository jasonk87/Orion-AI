...
function buildSectionsForPath({ service, workspacePath, relPath, query, symbols, include, contextLines }) {
  ...
  if (ranges.length === 0) {
    const fileTokens = estimateTokens(fileContext.source || lines.join('\n'));
    ranges.push({
      startLine: 1,
...
async function inspectCodeContext(workspacePath, request = {}) {
  ...
  const packet = service.createContextPacket({
  ...
  return {
    ...
    references,
    errors,
    contextPacketId: packet.success ? packet.packetId : '',
    contextPacket: packet.success ? packet : { success: false, error: packet.error },
    metrics: {
