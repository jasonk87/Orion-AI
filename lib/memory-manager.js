...
function globalFactCandidates(mem) {
  ...
  if (mem.user && Array.isArray(mem.user.preferences)) {
    for (const p of mem.user.preferences) candidates.push({ type: 'preference', text: p.text });
  }
...
async function appendGlobalPreference(text, config, source) {
  const mem = readGlobalMemory();
...
async function appendProjectPreference(workspacePath, text, config) {
  if (!text || !String(text).trim()) throw new Error('Preference text is required');
  const trimmed = String(text).trim();
  const mem = readProjectMemory(workspacePath);
  mem.preferences.push({ text: trimmed, addedAt: new Date().toISOString() });
  const result = writeProjectMemory(workspacePath, mem);
  await maybeEmbedFact(projectOrionDir(workspacePath), trimmed, config);
  return result;
}
