...
let lastSessionFileTimestampMs = 0;
...
function saveSessionMemory(workspacePath, sessionData) {
  ...
  // Multiple summaries can finish inside the same millisecond (especially on fast CI or when
  // several background conversations flush together). A timestamp-only filename silently
  // overwrote the earlier record. Keep filenames monotonically sortable and include the UUID so
  // each completed session always owns one durable file.
  const sortableTimestampMs = Math.max(Date.now(), lastSessionFileTimestampMs + 1);
  lastSessionFileTimestampMs = sortableTimestampMs;
  const timestamp = new Date(sortableTimestampMs).toISOString().replace(/[:.]/g, '-');
  const fileName = `${timestamp}-${sessionId}-session.json`;
