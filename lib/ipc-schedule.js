...
function registerHandlers(ipcMain, options = {}) {
  ...
  ipcMain.handle('orion:cancel-task-schedules', async (event, payload = {}) => {
    try { return ok(await store().cancelForSourceTask(payload.sourceTaskId)); } catch (error) { return fail(error); }
  });
  ...
  async function dispatchToRenderer(schedule, decision) {
    ...
    const payload = {
      ...
      deliveryOnly: schedule.deliveryOnly === true,
