...
// v3: cached vectors from the retired text-embedding-004 model are incompatible with the
// current embedding model — mixing them with fresh query vectors silently produces garbage
// similarity scores, so the cache version bump forces a clean re-embed.
...
async function generateEmbedding(text, config) {
  ...
  if (isOllama) {
    ...
    // DeepSeek has no embeddings API; the configured Gemini embedding model is the shared backend.
    ...
    const geminiModel = resolveGeminiEmbeddingModel(config);
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${geminiModel}:embedContent?key=${config.geminiApiKey}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function buildOrUpdateIndex(workspacePath, config) {
  ...
  const chunks = await service.getSemanticChunks(config, generateEmbedding);
  const index = {};
  for (const chunk of chunks) {
    const relPath = chunk.file;
    if (!index[relPath]) index[relPath] = {
      mtimeMs: service.getRecord(relPath) ? service.getRecord(relPath).mtimeMs : 0,
      size: service.getRecord(relPath) ? service.getRecord(relPath).size : 0,
      chunks: []
    };
    index[relPath].chunks.push(chunk);
  }
  return index;
}

async function buildOrUpdateIndexLegacy(workspacePath, config) {
...
module.exports = { semanticSearch, chunkText, chunkTextBySymbols, chunkLineRange, generateEmbedding, cosineSimilarity };
