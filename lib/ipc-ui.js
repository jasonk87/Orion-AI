...
const AUTO_UPDATE_FILES = [
  ...
  'dispatch-intent.js', 'orchestration-contracts.js', 'specialist-registry.js', 'task-orchestration.js',
  'prompt-submission.js', 'codex-provider.js',
  'lib/ipc-codex.js', 'lib/codex-subscription.js', 'lib/ipc-ollama.js',
  'workspace-resolution.js',
  'lib/ipc-file-tools.js', 'lib/ipc-shell.js', 'lib/ipc-computer-use.js', 'lib/chrome-favorites.js', 'lib/ipc-workspace.js',
  'lib/ipc-server.js', 'lib/ipc-ui.js', 'lib/ipc-skill.js', 'lib/ipc-memory.js',
  'lib/ipc-database.js', 'lib/atomic-json-store.js', 'lib/fault-log.js',
  ...
  'lib/workspace-index-client.js', 'lib/workspace-index-worker.js',
  'lib/conversation-memory.js', 'lib/ipc-orchestration.js', 'lib/orchestration-task-store.js',
  'lib/operator-control-session.js',
  'lib/ipc-schedule.js', 'lib/schedule-policy.js', 'lib/schedule-store.js',
  'lib/schedule-condition.js', 'lib/schedule-probe.js',
  'lib/ipc-resource-leases.js', 'lib/resource-lease-store.js', 'lib/screenshot-similarity.js',
  'lib/windows-window-capture.js'
];
...
// Root-level .js files that are build/test tooling, not app runtime. Mirrors the
// electron-packager --ignore list in package.json so the updater never copies them.
const NON_RUNTIME_ROOT_FILES = new Set(['test-runner.js']);

// The updater runs with the manifest baked into the CURRENTLY INSTALLED build. When an update
// introduces a brand-new module, that old manifest doesn't contain it: the updated files
...
// time (static manifest ∪ every lib/*.js and root runtime *.js present in the source) makes
// new modules ride along — both lib modules ("./scan-ignore") and root modules
// ("../task-orchestration") have caused this exact crash.
function resolveUpdateFileList(srcDir) {
  ...
  try {
    for (const entry of fs.readdirSync(srcDir)) {
      if (entry.endsWith('.js') && !NON_RUNTIME_ROOT_FILES.has(entry)
        && fs.statSync(path.join(srcDir, entry)).isFile()) {
        files.add(entry);
      }
    }
  } catch (_) {}
...
// Defaults to the SAME dynamic file list the update check uses, not the static manifest.
//
// The static AUTO_UPDATE_FILES list is a floor, not the runtime surface: resolveUpdateFileList
// adds every root *.js and lib/*.js present in the tree. Reading mtimes from the static list
// meant the version/date chip ignored real runtime files — reasoning-policy.js,
// semantic-intent-router.js and supervisor-orchestration.js among them — so editing only those
// left the displayed date unchanged and the app looked un-updated when it had in fact changed.
function getLatestRuntimeMtime(baseDir, files) {
  const base = baseDir || path.join(__dirname, '..');
  const fileList = Array.isArray(files) && files.length ? files : resolveUpdateFileList(base);
  let latest = 0;
...
async function checkForSourceUpdatesAndRelaunch() {
  ...
  try {
    ...
    // The packaged app resolves its update source by walking up out of dist/, which lands on
    // the repo it was built from. That is correct for a developer's local build and wrong for
    // an automated launch (a CI smoke test would be relaunched out from under itself), so the
    // check can be turned off explicitly.
    if (process.env.ORION_DISABLE_AUTO_UPDATE === '1') return false;
    const srcDir = resolveUpdateSourceDir();
